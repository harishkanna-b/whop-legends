...
class RateLimitLoadTester {
	...
	private async makeRequest(
		userId: string,
	): Promise<{ success: boolean; responseTime: number; blocked: boolean }> {
	...
	private async simulateUser(userId: number): Promise<void> {
		const rampUpDelay =
			(this.config.rampUpTime / this.config.concurrentUsers) * userId * 1000;
	...
	private calculateResults(): void {
		...
		this.results.p95ResponseTime = this.calculatePercentile(
			this.responseTimes,
			95,
		);
		this.results.p99ResponseTime = this.calculatePercentile(
			this.responseTimes,
			99,
		);
		this.results.requestsPerSecond =
			this.results.totalRequests / this.config.duration;
		this.results.errorRate =
			(this.results.blockedRequests / this.results.totalRequests) * 100;
	}

	async runTest(): Promise<LoadTestResult> {
		console.log(
			`🚀 Starting load test for ${this.config.endpointType} endpoint`,
		);
	...
	printResults(): void {
		...
		console.log(
			`Blocked Requests: ${this.results.blockedRequests.toLocaleString()}`,
		);
		console.log(
...
class SystemMonitor {
	...
	getStats(): any {
		...
		const memoryStats = {
			rss: {
				...
				avg:
					this.samples.reduce((sum, s) => sum + s.memory.rss, 0) /
					this.samples.length,
			},
			heapUsed: {
	...
	printStats(): void {
		...
		console.log(
			`   Min: ${(stats.memory.heapUsed.min / 1024 / 1024).toFixed(2)}MB`,
		);
		console.log(
			`   Max: ${(stats.memory.heapUsed.max / 1024 / 1024).toFixed(2)}MB`,
		);
		console.log(
			`   Avg: ${(stats.memory.heapUsed.avg / 1024 / 1024).toFixed(2)}MB`,
		);
		console.log(
			"═════════════════════════════════════════════════════════════════",
		);
...
async function runLoadTests() {
	...
	allResults.forEach(({ scenario, result }) => {
		if (result.errorRate > 20) {
			console.log(
				`⚠️  High error rate in ${scenario}. Consider increasing rate limits.`,
			);
		}
		if (result.p95ResponseTime > 100) {
			console.log(
				`⚠️  High response time in ${scenario}. Consider optimizing Redis usage.`,
			);
		}
		if (result.requestsPerSecond < 100) {
			console.log(
				`⚠️  Low throughput in ${scenario}. Check rate limiting configuration.`,
			);
		}
	});
