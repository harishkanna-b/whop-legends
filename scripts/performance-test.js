...
async function testRateLimitingPerformance() {
	...
	try {
		...
		console.log(
			`   - Requests per second: ${(1000 / (totalTime / 1000)).toFixed(2)}`,
		);
...
async function testSignaturePerformance() {
	...
	try {
		...
		} = require("../lib/webhooks/verify-signature");
		...
		console.log(
			`   - Verifications per second: ${(iterations / (totalTime / 1000)).toFixed(2)}`,
		);
...
async function testMemoryUsage() {
	...
	console.log(
		`   - Heap Used: ${(loadedMemory.heapUsed / 1024 / 1024).toFixed(2)} MB`,
	);
	console.log(
		`   - Heap Total: ${(loadedMemory.heapTotal / 1024 / 1024).toFixed(2)} MB`,
	);
...
async function testDatabasePerformance() {
	...
	try {
		...
		if (error) {
			console.log(
				"⚠️  Database query error (expected if not connected):",
				error.message,
			);
			return { success: false, error: error.message };
		}
...
async function runPerformanceTests() {
	...
	if (results.rateLimiting.success) {
		console.log(
			`✅ Rate Limiting: ${results.rateLimiting.requestsPerSecond.toFixed(2)} req/sec`,
		);
	} else {
	...
	if (results.signature.success) {
		console.log(
			`✅ Signature Verification: ${results.signature.verificationsPerSecond.toFixed(2)} verifications/sec`,
		);
	} else {
		console.log(
			`❌ Signature Verification: Failed - ${results.signature.error}`,
		);
	}

	if (results.memory.success) {
		console.log(
			`✅ Memory Usage: ${(results.memory.heapIncrease / 1024 / 1024).toFixed(2)} MB increase under load`,
		);
	} else {
	...
	if (results.database.success) {
		console.log(
			`✅ Database Query: ${results.database.queryTime.toFixed(2)}ms`,
		);
	} else {
