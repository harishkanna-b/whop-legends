...
const config = {
  ...
  duration: parseInt(process.env.TEST_DURATION) || 30000, // 30 seconds
...
async function makeWebhookRequest() {
  ...
  return new Promise((resolve) => {
    ...
    stats.maxConcurrentRequests = Math.max(stats.maxConcurrentRequests, stats.concurrentRequests);
    ...
    timeoutTimer = setTimeout(() => {
      cleanup(performance.now() - startTime, new Error(`Request exceeded ${config.requestTimeout}ms timeout`));
    }, config.requestTimeout);

    const req = (new URL(config.baseUrl).protocol === 'https:' ? https : http).request(options, (res) => {
    ...
    req.setTimeout(config.requestTimeout, () => {
      const error = new Error(`Request socket timeout after ${config.requestTimeout}ms`);
    ...
    req.on('close', () => {
      if (!isResolved) {
        ...
        cleanup(performance.now() - startTime, new Error('Request closed unexpectedly'));
      }
    });
...
function calculateStats() {
  ...
  return {
    ...
    rateLimitRate: (stats.rateLimitedRequests / stats.totalRequests * 100).toFixed(2),
    ...
    medianResponseTime: sortedTimes.length > 0 ? sortedTimes[Math.floor(sortedTimes.length / 2)].toFixed(2) : 0,
    p95ResponseTime: sortedTimes.length > 0 ? sortedTimes[Math.floor(sortedTimes.length * 0.95)].toFixed(2) : 0,
    p99ResponseTime: sortedTimes.length > 0 ? sortedTimes[Math.floor(sortedTimes.length * 0.99)].toFixed(2) : 0,

    // Concurrency statistics
    maxConcurrentRequests: stats.maxConcurrentRequests,
    avgConcurrentRequests: stats.concurrencySamples.length > 0 ? (stats.concurrencySamples.reduce((sum, sample) => sum + sample, 0) / stats.concurrencySamples.length).toFixed(2) : 0,

    // Requests per second
    duration: stats.endTime - stats.startTime,
    requestsPerSecond: stats.endTime > stats.startTime ? (stats.totalRequests / ((stats.endTime - stats.startTime) / 1000)).toFixed(2) : 0,
...
function reportProgress(label = '') {
  ...
  console.log(`Successful: ${currentStats.successfulRequests} (${currentStats.successRate}%)`);
  console.log(`Failed: ${currentStats.failedRequests} (${currentStats.failureRate}%)`);
  console.log(`Rate Limited: ${currentStats.rateLimitedRequests} (${currentStats.rateLimitRate}%)`);
...
async function runLoadTest() {
  ...
  const progressInterval = setInterval(() => {
    ...
    if (stats.lastSampleTime === null || Date.now() - stats.lastSampleTime >= 1000) {
  ...
  for (let i = 0; i < config.concurrentUsers; i++) {
    requestPromises.push(
      (async function requestWorker() {
        while (Atomics.load(sharedCompletedRequests, 0) < config.totalRequests && (Date.now() - stats.startTime) < config.duration) {
          activeRequests++;
  ...
  setInterval(() => {
    stats.maxConcurrentRequests = Math.max(stats.maxConcurrentRequests, maxActiveRequests);
  }, 1000);
