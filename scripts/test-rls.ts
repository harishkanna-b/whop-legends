...
async function testRLS() {
  try {
    ...
    if (classesError) {
      ...
      console.log('✅ Can access character classes (public):', classes?.length || 0, 'classes');
    }
    ...
    if (usersError) {
      console.log('✅ Correctly blocked from accessing users table:', usersError.message);
    } else if (users && users.length === 0) {
      console.log('✅ Correctly returns empty result (no user data accessible without auth)');
    } else {
      console.error('❌ ERROR: Should not be able to access user data without auth');
    }
    ...
    if (insertError) {
      console.log('✅ Correctly blocked from inserting into users table:', insertError.message);
    } else {
      console.error('❌ ERROR: Should not be able to insert into users table without auth');
    }
    ...
    if (questsError) {
      ...
      console.log('✅ Can access quests (public):', quests?.length || 0, 'quests');
    }
