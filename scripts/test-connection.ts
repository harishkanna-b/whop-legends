...
if (!supabaseUrl || !supabaseAnonKey) {
  ...
  console.log('1. Go to https://supabase.com/dashboard/project/slftjqvrjdkzvzenmvnq');
...
async function testConnection() {
  try {
    ...
    if (error) {
      ...
      if (error.message.includes('relation "character_classes" does not exist')) {
        console.log('💡 This is expected - the database schema hasn\'t been created yet.');
        console.log('📝 To set up the database:');
        console.log('1. Go to your Supabase dashboard');
        console.log('2. Navigate to the SQL Editor');
        console.log('3. Copy the schema from supabase/migrations/001_initial_schema.sql');
