...
export async function POST(request: NextRequest): Promise<Response> {
	...
	if (webhookData.action === "payment.succeeded") {
		const { id, final_amount, amount_after_fees, currency, user_id, member } =
			webhookData.data;
...
export async function paymentReferralHandler(
	...
	try {
		if (!userId) {
			console.log("No user_id in payment webhook, skipping referral processing");
			return;
		}
		...
		if (!referrerId) {
			console.log(`No referrer found for user ${userId}, processing as direct payment`);
			await processDirectPayment(userId, amount, currency);
			return;
		}

		console.log(`Processing referral: user ${userId} referred by ${referrerId} for payment ${amount} ${currency}`);

		// Process referral commission
		await processReferralCommission(referrerId, userId, amount, paymentId, member);
	...
	try {
		...
		if (user?.metadata?.referred_by) {
			...
			await referralManager.createReferral(
				referrerId,
				userId,
				amount, // This will be updated when we have the payment amount
				"whop_payment",
				{
					payment_id: "pending", // Will be updated with actual payment ID
...
async function processReferralCommission(
	...
	try {
		...
		console.log(`Referral commission processed: ${commission} for referrer ${referrerId}`);
...
async function updateReferrerQuestProgress(referrerId: string, amount: number) {
	try {
		...
		const referralQuests = userQuests.filter(uq =>
...
async function processDirectPayment(userId: string, amount: number, currency: string) {
	try {
		...
		if (userQuests && userQuests.length > 0) {
			const paymentQuests = userQuests.filter(uq =>
		...
		console.log(`Processed direct payment for user ${userId}: ${amount} ${currency}`);
...
async function updateUserCommissionStats(userId: string, commission: number) {
	try {
		await supabaseService
			.from("users")
			.update({
				total_commission: supabaseService.raw(`total_commission + ${commission}`),
				updated_at: new Date().toISOString()
			})
			.eq("id", userId);
