...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!user_id || !action_type) {
      return createErrorResponse('Missing required fields: user_id, action_type', 400);
    }

    // Validate action_type
    const validActionTypes = ['referral', 'commission', 'level', 'achievement'];
    if (!validActionTypes.includes(action_type)) {
      return createErrorResponse(`Invalid action_type: ${action_type}. Must be one of: ${validActionTypes.join(', ')}`, 400);
    }

    // Validate value
    const progressValue = value || 1;
    if (typeof progressValue !== 'number' || progressValue < 0 || progressValue > 1000) {
      return createErrorResponse('Value must be a number between 0 and 1000', 400);
    }
