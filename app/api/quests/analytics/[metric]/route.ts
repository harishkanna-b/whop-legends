...
async function getPerformanceAnalytics(
  ...
  topPerformers?.forEach((item) => {
    ...
    userStats[item.user_id].total_commission += quest?.[0]?.reward_commission || 0;
  });
...
async function getRewardAnalytics(
  ...
  const totalXP = claimedRewards.reduce(
    (sum, item) => sum + ((item.quest as any)?.[0]?.reward_xp || 0),
    0,
  );
  const totalCommission = claimedRewards.reduce(
    (sum, item) => sum + ((item.quest as any)?.[0]?.reward_commission || 0),
    0,
  );
  ...
  claimedRewards.forEach((item) => {
    ...
    if (questData) {
      ...
      if (!rewardsByType[questData.quest_type]) {
        rewardsByType[questData.quest_type] = { xp: 0, commission: 0, count: 0 };
      }
      rewardsByType[questData.quest_type].xp += questData.reward_xp;
      rewardsByType[questData.quest_type].commission += questData.reward_commission;
