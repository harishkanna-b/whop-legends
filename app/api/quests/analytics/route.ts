...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: questStats, error: questError } = await supabaseService
    ...
    const { data: engagementData, error: engagementError } = await supabaseService
      .from('user_quests')
    ...
    const analytics = {
      ...
      completion_trends: await getCompletionTrends(companyId, questType, startDate, now)
    };
 ...
 */
async function getFallbackAnalytics(companyId: string, questType: string | null, startDate: Date, endDate: Date) {
  try {
    ...
    const filteredUserQuests = userQuests.filter(uq =>
      filteredQuests.some(q => q.id === uq.quest_id)
    );

    const totalQuests = filteredQuests.length;
    const completedQuests = filteredUserQuests.filter(uq => uq.is_completed).length;
    const totalUsers = new Set(filteredUserQuests.map(uq => uq.user_id)).size;
    ...
    const analytics = {
      ...
      completion_trends: await getCompletionTrends(companyId, questType, startDate, endDate)
    };
 ...
 */
async function getPopularQuests(companyId: string, startDate: Date, endDate: Date) {
  try {
    ...
    const questCounts: Record<string, { quest: any, completions: number, attempts: number }> = {};
 ...
 */
async function getCompletionTrends(companyId: string, questType: string | null, startDate: Date, endDate: Date) {
  try {
    const { data, error } = await supabaseService
      .from('user_quests')
      .select('created_at, completed_at, is_completed, quest:quests(quest_type)')
      .gte('created_at', startDate.toISOString())
      .lte('created_at', endDate.toISOString())
      .eq('quest.company_id', companyId);
