...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user_id || !user_quest_id) {
      return createErrorResponse('Missing required fields: user_id, user_quest_id', 400);
    }

    const reward = await QuestEngine.claimQuestRewards(user_id, user_quest_id);

    if (!reward) {
      return createErrorResponse('Quest not found, not completed, or rewards already claimed', 404);
    }
