...
import { QuestFilters } from '@/lib/types/quest-types';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { user_id, company_id, user_level, character_class, quest_type } = body;

    if (!user_id || !company_id || !quest_type) {
      return createErrorResponse('Missing required fields: user_id, company_id, quest_type', 400);
    }
