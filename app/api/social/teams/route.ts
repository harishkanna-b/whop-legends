		...
		switch (action) {
			...
			case "team":
				...
				return NextResponse.json({
					...
					data: {
						teamId,
						message:
							"Team details endpoint not yet implemented (mock response)",
					},
				});
			...
			case "discover":
				...
				return NextResponse.json({
					...
					data: {
						userId,
						message:
							"Discoverable teams endpoint not yet implemented (mock response)",
						teams: [],
					},
				});
			...
			case "join-via-code": {
				...
				return NextResponse.json({
					...
					data: {
						...
						inviteCode,
						message:
							"Join team via code endpoint not yet implemented (mock response)",
					},
			...
			default:
				return NextResponse.json(
					{
						error:
							"Invalid action. Use: my-teams, team, discover, or join-via-code",
					},
					{ status: 400 },
				);
		}
