...
async function handler(request: NextRequest) {
	try {
		...
		const timeframe =
			(searchParams.get("timeframe") as "7d" | "30d" | "90d" | "1y" | "all") ||
			"all";
		...
		switch (action) {
			...
			case "referrals": {
				const referrals = await referralManager.getUserReferrals(
					userId,
					timeframe,
				);
...
export const GET = async (request: NextRequest) => {
	...
	const rateLimitResult = await rateLimitMiddleware(request as any, {
		...
		keyGenerator: (req) =>
			`referrals:${req.headers["x-forwarded-for"] || req.connection.remoteAddress || "unknown"}`,
	});
