...
export default function ReferralTracking({
	...
	const [activeTab, setActiveTab] = useState<
		"overview" | "referrals" | "campaigns" | "analytics"
	>("overview");
	...
	const loadReferralData = async () => {
		try {
			...
			const referralsData = await referralManager.getUserReferrals(
				userId,
				selectedTimeframe,
			);
			...
			const statsData = await referralManager.getReferralStats(
				userId,
				selectedTimeframe,
			);
			setStats(statsData);
	...
	const generateReferralLink = async (campaignId?: string) => {
		try {
			const link = await referralManager.generateReferralLink(
				userId,
				campaignId,
			);
			// Copy to clipboard
				...
				<div className="bg-white rounded-lg shadow mb-8">
					<div className="border-b border-gray-200">
						<nav className="flex space-x-8 px-6">
							{(
								["overview", "referrals", "campaigns", "analytics"] as const
							).map((tab) => (
								<button
									key={tab}
												...
												{referrals.slice(0, 5).map((referral) => (
													<tr key={referral.id}>
														...
														<td className="px-6 py-4 whitespace-nowrap">
															<span
																className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(referral.status)}`}
															>
										...
										>
											...
											<div className="space-y-2 text-sm">
												...
												<div className="flex justify-between">
													...
													<span className="font-medium">
														${campaign.currentValue.toLocaleString()}
													</span>
												</div>
												...
												<div key={month.month}>
													...
													<div className="w-full bg-gray-200 rounded-full h-2">
														<div
															...
															style={{
																width: `${(month.referrals / Math.max(...stats.monthlyTrend.map((m) => m.referrals))) * 100}%`,
															}}
														/>
													</div>
												</div>
