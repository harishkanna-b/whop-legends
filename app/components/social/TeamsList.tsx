...
import { Team, TeamInvite } from '@/lib/social'
...
export default function TeamsList({ userId }: TeamsListProps) {
  ...
  const [activeTab, setActiveTab] = useState<'my-teams' | 'discover' | 'invites'>('my-teams')
  ...
  const loadTeams = async () => {
    try {
      const response = await fetch(`/api/social/teams?userId=${userId}&action=my-teams`)
  ...
  const loadDiscoverableTeams = async () => {
    try {
      const response = await fetch(`/api/social/teams?userId=${userId}&action=discover`)
      const result = await response.json()
      if (result.success) {
  ...
  const joinTeamViaCode = async (inviteCode: string) => {
    try {
      const response = await fetch(`/api/social/teams?userId=${userId}&action=join-via-code&inviteCode=${inviteCode}`)
      const result = await response.json()
      if (result.success) {
