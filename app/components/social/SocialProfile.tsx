...
import type { SocialProfile, SocialActivity } from '@/lib/social'
...
export default function SocialProfile({ userId, isOwnProfile = false }: SocialProfileProps) {
  ...
  const loadProfile = async () => {
    try {
      const response = await fetch(`/api/social/profile?userId=${userId}&action=get`)
  ...
  const loadActivity = async () => {
    try {
      const response = await fetch(`/api/social/profile?userId=${userId}&action=activity&limit=10`)
      const result = await response.json()
      if (result.success) {
    ...
    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                ...
                <>
                  ...
                  <div className="flex items-center space-x-4 mt-2 text-sm text-gray-500">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className={`w-2 h-2 rounded-full ${getStatusColor(profile.activityStatus)}`}></div>
      ...
      <div className="border-t border-gray-200 px-6 py-4">
        <div className="grid grid-cols-4 gap-4 text-center">
          <div>
            <div className="text-2xl font-bold text-blue-600">{profile.stats?.friendsCount || 0}</div>
            <div className="text-sm text-gray-500">Friends</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-purple-600">{profile.stats?.teamsCount || 0}</div>
            <div className="text-sm text-gray-500">Teams</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-green-600">{profile.stats?.achievementsCount || 0}</div>
            <div className="text-sm text-gray-500">Achievements</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-orange-600">{profile.stats?.totalActivity || 0}</div>
