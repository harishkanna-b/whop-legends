...
import type { Friend, FriendRequest } from "@/lib/social";
...
export default function FriendsList({ userId }: FriendsListProps) {
	...
	const loadFriends = async () => {
		try {
			const response = await fetch(
				`/api/social/friends?userId=${userId}&action=list`,
			);
	...
	const loadRequests = async () => {
		try {
			const response = await fetch(
				`/api/social/friends?userId=${userId}&action=requests`,
			);
			const result = await response.json();
			if (result.success) {
	...
	const removeFriend = async (friendId: string) => {
		try {
			const response = await fetch(
				`/api/social/friends/remove?userId=${userId}&friendId=${friendId}`,
				{
					method: "DELETE",
				},
			);
						...
						friends.map((friend) => (
							...
							>
								<div className="flex items-center space-x-4">
									...
									<div>
										...
										<p className="text-xs text-gray-400">
											Friends since{" "}
											{new Date(friend.requestedAt).toLocaleDateString()}
										</p>
									</div>
								</div>
								<div className="flex space-x-2">
									...
									<button
										onClick={() =>
											(window.location.href = `/profile/${friend.friendId}`)
										}
