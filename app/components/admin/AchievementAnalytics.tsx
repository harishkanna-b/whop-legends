			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-6">
				...
				<div className="bg-white rounded-lg shadow p-6">
					<div className="text-3xl font-bold text-green-600">
						{stats.unlockedAchievements.toLocaleString()}
					</div>
					<div className="text-sm text-gray-600">Unlocked</div>
				</div>
				<div className="bg-white rounded-lg shadow p-6">
					<div className="text-3xl font-bold text-purple-600">
						{stats.unlockRate.toFixed(1)}%
					</div>
						...
						>
							<div className="flex items-center space-x-4">
								...
								<div>
									...
									<div className="text-sm text-gray-600">
										...
										<span
											className={`ml-1 ${getRarityColor(achievement.rarity)}`}
										>
											{achievement.rarity}
										</span>
									</div>
							...
							<div className="text-right">
								<div className="font-semibold text-green-600">
									{achievement.unlocks.toLocaleString()}
								</div>
							...
							<div className="flex items-center justify-between mb-2">
								...
								<div
									className={`text-sm font-semibold ${
										category.unlockRate > 80
											? "text-green-600"
											: category.unlockRate > 60
												? "text-yellow-600"
												: category.unlockRate > 40
													? "text-orange-600"
													: "text-red-600"
									}`}
								>
							...
							<div className="space-y-2">
								...
								<div className="flex justify-between text-sm">
									...
									<span className="font-medium">
										{category.unlocks.toLocaleString()}
									</span>
								</div>
					...
					<div>
						...
						<div className="space-y-2">
							{["common", "uncommon", "rare", "epic", "legendary"].map(
								(rarity) => {
									const rarityCount = stats.topAchievements.filter(
										(a) => a.rarity === rarity,
									).length;
									const percentage =
										(rarityCount / stats.topAchievements.length) * 100;
									return (
										<div
											key={rarity}
										...
										>
											<span
												className={`text-sm font-medium ${getRarityColor(rarity)}`}
											>
