...
export default function LevelProgression({ userId }: LevelProgressionProps) {
	...
	const loadProgressionData = async () => {
		try {
			...
			const xpForCurrentLevel = LevelingManager.calculateXPRequired(
	...
	const getProgressPercentage = () => {
		...
		return Math.min(
			(progress.currentLevelXP / progress.xpToNextLevel) * 100,
			100,
		);
	};
				...
				<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
					...
					<div className="bg-purple-50 rounded-lg p-4 text-center">
						<div className="text-2xl font-bold text-purple-600">
							{progress.characterClass
								? `${progress.characterClass.xpMultiplier}x`
								: "1.0x"}
						</div>
							...
							{xpHistory.map((event) => (
								...
								>
									<div className="flex items-center space-x-3">
										<div
											className={`w-8 h-8 rounded-full flex items-center justify-center ${getEventTypeColor(event.type)}`}
										>
									...
									<div className="text-right">
										<p
											className={`font-bold ${event.amount > 0 ? "text-green-600" : "text-red-600"}`}
										>
											{event.amount > 0 ? "+" : ""}
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					<div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4">
						<h5 className="font-semibold text-gray-900 mb-2">
							Level {progress.level + 1} Benefits
						</h5>
