...
import { MemberDirectory as MemberDirectoryService, MemberFilter, MemberSearchResult } from '@/lib/analytics/member-directory';
...
export function MemberDirectory({ companyId }: MemberDirectoryProps) {
  ...
  const exportMembers = async (format: 'csv' | 'json' | 'xlsx' = 'csv') => {
    try {
      const exportOptions = {
        format,
        fields: ['username', 'character_class', 'level', 'total_referrals', 'total_commission', 'conversion_rate', 'engagement_score'],
        filters
      };

      const data = await memberDirectory.exportMembers(companyId, exportOptions);
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Manage and analyze your community members</p>
        </div>
      ...
      {showFilters && (
        <div className="bg-white rounded-lg shadow p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Character Class</label>
              <select
                value={filters.character_class || ''}
                onChange={(e) => handleFilterChange('character_class', e.target.value || undefined)}
                className="w-full border border-gray-300 rounded-lg px-3 py-2"
              >
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="text-center">
            ...
            <p className="text-2xl font-bold text-gray-900">
              {formatCurrency(members.reduce((sum, m) => sum + m.total_commission, 0) / (members.length || 1))}
            </p>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-6">
          <div className="text-center">
            ...
            <p className="text-2xl font-bold text-gray-900">
              {formatPercentage(members.reduce((sum, m) => sum + m.conversion_rate, 0) / (members.length || 1))}
            </p>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-6">
          <div className="text-center">
            ...
            <p className="text-2xl font-bold text-gray-900">
              {formatPercentage(members.reduce((sum, m) => sum + m.engagement_score, 0) / (members.length || 1))}
            </p>
          </div>
        </div>
      </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {members.map((member) => (
                <tr key={member.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <span className="text-lg mr-2">{getClassIcon(member.character_class)}</span>
                      <div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center space-x-2">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getEngagementColor(member.engagement_score)}`}>
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                ...
                of <span className="font-medium">{formatNumber(total)}</span> results
              </p>
            </div>
            <div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{selectedMember.character_class}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Level</span>
                      <span className="font-medium">{selectedMember.level}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Rank</span>
                      <span className="font-medium">#{selectedMember.rank}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Status</span>
                      <span className={`font-medium ${
                        selectedMember.is_active ? 'text-green-600' : 'text-red-600'
                      }`}>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{formatCurrency(selectedMember.total_commission)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Total Referrals</span>
                      <span className="font-medium">{formatNumber(selectedMember.total_referrals)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Conversion Rate</span>
                      <span className="font-medium">{formatPercentage(selectedMember.conversion_rate)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Engagement Score</span>
                      <span className="font-medium">{formatPercentage(selectedMember.engagement_score)}</span>
                    </div>
                  </div>
                </div>
              </div>
