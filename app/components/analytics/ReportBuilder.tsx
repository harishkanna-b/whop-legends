...
export function ReportBuilder({ companyId }: ReportBuilderProps) {
	...
	const fetchRecentReports = async () => {
		try {
			...
			const response = await fetch(
				`/api/reports/generate?companyId=${companyId}`,
			);
			if (response.ok) {
				...
				const updatedTemplates = predefinedTemplates.map((template) => {
					const recentReport = reports.find(
						(r: any) => r.report_type === template.type,
					);
					return {
						...template,
	...
	const downloadReport = async (template: ReportTemplate) => {
		try {
			const response = await fetch(
				`/api/reports/generate?companyId=${companyId}&reportType=${template.type}`,
			);
			if (response.ok) {
				const report = await response.json();
						...
						<div className="space-y-2 mb-4">
							<div className="flex justify-between text-sm">
								...
								<span className="font-medium">
									{formatTimeframe(template.timeframe)}
								</span>
							</div>
							<div className="flex justify-between text-sm">
			...
			<div className="bg-white rounded-lg shadow">
				...
				<div className="p-6">
					<div className="space-y-3">
						...
						{reportTemplates.filter((template) => template.lastGenerated)
							.length === 0 && (
