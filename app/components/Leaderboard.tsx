...
export default function Leaderboard({
	...
	const loadLeaderboard = async () => {
		try {
			...
			const data = await LevelingManager.getLeaderboard(
				limit,
			...
			const processedData = data.map((entry: any, index: number) => ({
				...
				characterClass: entry.classId
					? CharacterClassManager.getClassById(entry.classId)
					: null,
				stats: {
		...
		<div className="bg-white rounded-lg shadow-lg overflow-hidden">
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						<div>
							...
							<select
								...
								onChange={(e) =>
									setFilters({ ...filters, timeFrame: e.target.value as any })
								}
						...
						<div>
							...
							<select
								...
								onChange={(e) =>
									setFilters({
										...filters,
										classFilter: e.target.value || null,
									})
								}
								className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
							>
						...
						<div>
							...
							<select
								...
								onChange={(e) =>
									setFilters({ ...filters, metric: e.target.value as any })
								}
								className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
							>
				...
				{sortedLeaderboard.length > 0 ? (
							...
							>
								...
								<div className="flex-shrink-0 w-12 h-12 flex items-center justify-center">
									{entry.rank <= 3 ? (
										...
										<span
											className={`text-lg font-bold ${entry.isCurrentUser ? "text-blue-600" : "text-gray-600"}`}
										>
											{entry.rank}
										</span>
									)}
								...
								{/* Character Info */}
								<div
									className="flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center text-xl ml-4"
									style={{
								...
								<div className="flex-1 ml-4">
									<div className="flex items-center space-x-2">
										<h4
											className={`font-bold ${entry.isCurrentUser ? "text-blue-900" : entry.rank <= 3 ? "text-white" : "text-gray-900"}`}
										>
											{entry.username}
										</h4>
										{entry.isCurrentUser && (
									...
									<div
										className={`text-sm ${entry.rank <= 3 ? "text-blue-100" : "text-gray-500"}`}
									>
										Level {entry.level} • {entry.stats.totalReferrals} referrals
										• ${entry.stats.totalValue.toLocaleString()} value
									</div>
								</div>
								...
								<div className="flex-shrink-0 text-right ml-4">
									<div
										className={`text-lg font-bold ${entry.isCurrentUser ? "text-blue-600" : entry.rank <= 3 ? "text-white" : "text-gray-900"}`}
									>
										{formatMetric(
											filters.metric === "level"
												? entry.level
												: filters.metric === "xp"
													? entry.totalXP
													: filters.metric === "referrals"
														? entry.stats.totalReferrals
														: entry.stats.totalValue,
											filters.metric,
										)}
									</div>
									<div
										className={`text-xs ${entry.rank <= 3 ? "text-blue-100" : "text-gray-500"}`}
									>
										{filters.metric === "level"
											? "Level"
											: filters.metric === "xp"
					...
					<div className="text-center py-12">
						...
						<p className="text-gray-500">
							Be the first to start climbing the ranks!
						</p>
					</div>
				)}
			</div>
			...
			<div className="border-t border-gray-200 px-6 py-4 bg-gray-50">
				<div className="flex items-center justify-between text-sm text-gray-600">
					<div>
						Showing top {Math.min(sortedLeaderboard.length, limit)} of{" "}
						{sortedLeaderboard.length} legends
					</div>
