...
export function LiveRankingUpdates({
  ...
  const [userRankUpdate, setUserRankUpdate] = useState<RankingUpdate | null>(null);

  // Convert WebSocket messages to ranking updates
  useEffect(() => {
    const rankingUpdates: RankingUpdate[] = leaderboardData.rankUpdates.map(update => {
    ...
    const currentUserUpdate = rankingUpdates.find(update => update.userId === currentUserId);
  ...
  return (
    <div className="space-y-4">
      ...
      {userRankUpdate && (
        <div className={`p-4 rounded-lg border-2 ${getChangeColor(userRankUpdate.changeType)} animate-pulse`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <span className="text-2xl">{getChangeIcon(userRankUpdate.changeType)}</span>
              <div>
                ...
                <p className="text-sm text-gray-600">
                  ...
                  {userRankUpdate.scoreChange !== 0 && (
                    <span className="ml-2">
                      ({userRankUpdate.scoreChange > 0 ? '+' : ''}{userRankUpdate.scoreChange})
                    </span>
                  )}
                </p>
              </div>
          ...
          {recentUpdates.length === 0 ? (
            ...
            <div className="divide-y divide-gray-200">
              {recentUpdates.map((update, index) => (
                <div key={`${update.userId}-${index}`} className="p-3 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <span className="text-lg">{getChangeIcon(update.changeType)}</span>
                      <div>
                        ...
                        <p className="text-xs text-gray-600">
                          ...
                          {update.scoreChange !== 0 && (
                            <span className="ml-1">
                              ({update.scoreChange > 0 ? '+' : ''}{update.scoreChange} pts)
                            </span>
                          )}
                        </p>
                      </div>
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        ...
        <ul className="text-sm text-yellow-800 space-y-1">
          ...
          <li>• Focus on activities that boost your score in your preferred category</li>
          <li>• Character class multipliers can significantly impact your ranking</li>
