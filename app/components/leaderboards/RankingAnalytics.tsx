...
export function RankingAnalytics({
	...
	const analyticsData: AnalyticsData = useMemo(() => {
		...
		const userPercentile = userRank
			? ((totalParticipants - userRank) / totalParticipants) * 100
			: undefined;
		...
		const scoreRanges = [
			...
			{ min: averageScore * 1.5, max: Number.POSITIVE_INFINITY, label: "Top" },
		];
		...
		const rankRanges = [
			...
			{ min: 101, max: Number.POSITIVE_INFINITY, label: "Others" },
		];

		const rankDistribution = rankRanges.map((range) => ({
			range: range.label,
			count: entries.filter((e) => e.rank >= range.min && e.rank <= range.max)
				.length,
			percentage:
				(entries.filter((e) => e.rank >= range.min && e.rank <= range.max)
					.length /
					totalParticipants) *
				100,
		}));
			...
			{selectedView === "class" && (
				<div className="bg-white p-6 rounded-lg shadow border border-gray-200">
					...
					<div className="space-y-3">
						{Object.entries(analyticsData.classDistribution).map(
							([className, count]) => (
								<div
									key={className}
			...
			<div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg border border-blue-200">
				...
				<ul className="space-y-2 text-sm text-blue-800">
					<li className="flex items-start space-x-2">
						...
						<span>
							Most participants are in the{" "}
							{
								analyticsData.scoreDistribution.reduce((max, curr) =>
					...
					<li className="flex items-start space-x-2">
						...
						<span>
							The{" "}
							{
								Object.entries(analyticsData.classDistribution).reduce(
									(max, [className, count]) =>
										count > max[1] ? [className, count] : max,
									["", 0],
								)[0]
							}{" "}
							class has the most participants
						</span>
					</li>
					<li className="flex items-start space-x-2">
						<span className="text-blue-600 mt-0.5">•</span>
						<span>
							Top 10% of participants are in the top{" "}
							{Math.round(analyticsData.totalParticipants * 0.1)} ranks
						</span>
					</li>
					{analyticsData.userPercentile && (
						<li className="flex items-start space-x-2">
							...
							<span>
								...
								{Math.round(analyticsData.userPercentile)}% of participants
							</span>
						</li>
					)}
				</ul>
