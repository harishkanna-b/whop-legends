...
export function LeaderboardDisplay({
	...
	const fetchLeaderboard = async () => {
		try {
			...
			const response = await fetch(
				`/api/leaderboards/${category}?companyId=${companyId}&timeframe=${timeframe}&limit=${limit}&page=${page}`,
			);
	...
	const refreshLeaderboard = async () => {
		try {
			const response = await fetch(
				`/api/leaderboards/${category}?companyId=${companyId}&timeframe=${timeframe}&action=refresh`,
				{
					method: "POST",
				},
			);
	...
	const fetchUserRank = async () => {
		try {
			const response = await fetch(
				`/api/rankings/${category}/${timeframe}?companyId=${companyId}&userId=${userRank?.user_id}`,
			);
					...
					{showUserRank && userRank && (
						<div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-2">
							<span className="text-sm text-blue-800">
								...
								{((userRank.rank / entries.length) * 100).toFixed(1)}th
								percentile)
							</span>
						</div>
					)}
								...
								>
									...
									<td className="px-6 py-4 whitespace-nowrap">
										<div className="text-sm text-gray-900">
											{category === "referrals" &&
												`${entry.metrics.total_referrals} referrals`}
											{category === "commission" &&
												`$${entry.metrics.total_commission.toLocaleString()}`}
											{category === "engagement" &&
												`${entry.metrics.engagement_score.toFixed(1)}% engagement`}
											{category === "quests" &&
												`${entry.metrics.quest_completion_rate.toFixed(1)}% completion`}
											{category === "retention" &&
												`${entry.metrics.retention_rate.toFixed(1)}% retention`}
				...
				{totalEntries > limit && (
					<div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
						<div className="flex-1 flex justify-between sm:hidden">
							...
							<button
								onClick={() =>
									setPage(Math.min(Math.ceil(totalEntries / limit), page + 1))
								}
						...
						<div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
							...
							<div>
								<nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
									...
									{Array.from(
										{ length: Math.min(5, Math.ceil(totalEntries / limit)) },
										(_, i) => {
											const pageNum =
												Math.max(
													1,
													Math.min(
														Math.ceil(totalEntries / limit) - 4,
														page - 2,
													),
												) + i;
											return (
									...
									<button
										onClick={() =>
											setPage(
												Math.min(Math.ceil(totalEntries / limit), page + 1),
											)
										}
										disabled={page >= Math.ceil(totalEntries / limit)}
