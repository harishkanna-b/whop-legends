...
export function QuestHistory({ userId }: QuestHistoryProps) {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const response = await fetch(`/api/quests/history?userId=${userId}&limit=${limit}&offset=${offset}`, {
        method: 'GET',
        headers: {
    ...
    <div className="space-y-4">
      ...
      {history.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">No Quest History</h3>
          <p className="text-gray-600">Complete some quests to see your history here!</p>
        </div>
      ) : (
        <>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-3 mt-2">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getDifficultyColor(historyQuest.quest.difficulty)}`}>
                            {historyQuest.quest.difficulty.charAt(0).toUpperCase() + historyQuest.quest.difficulty.slice(1)}
                          </span>
                          <span className="text-xs text-gray-500">
                            Completed: {historyQuest.completed_at ? formatDate(historyQuest.completed_at) : 'Unknown'}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <div className="text-lg font-semibold text-gray-900">
                        {historyQuest.progress_value} / {historyQuest.quest.target_value}
                      </div>
                      <div className="mt-2 space-y-1">
                        ...
                        {historyQuest.reward_claimed && (
                          <div className="text-xs text-purple-600">
                            💎 Claimed: {historyQuest.reward_claimed_at ? formatDate(historyQuest.reward_claimed_at) : 'Unknown'}
                          </div>
                        )}
                      </div>
                    </div>
