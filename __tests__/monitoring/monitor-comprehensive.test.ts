import { MonitoringService, monitoring, MetricData, HealthCheck, AlertRule, Alert, monitorPerformance, healthCheck } from '@/lib/monitoring/monitor';
...
describe('Monitoring System - Comprehensive Coverage', () => {
  ...
  describe('Metrics Collection', () => {
    ...
    it('should collect histogram metrics', () => {
      monitoringService.histogram('response.time', 150, { endpoint: '/api/users' });
    ...
    it('should collect timer metrics', () => {
      monitoringService.timer('operation.duration', 250, { operation: 'database.query' });
  ...
  describe('Performance Monitoring', () => {
    ...
    it('should handle timing with tags', async () => {
      ...
      await monitoringService.time(
        'tagged.operation',
        mockOperation,
        { environment: 'test', component: 'auth' }
      );
  ...
  describe('Health Check Management', () => {
    ...
    it('should call health logger', async () => {
      ...
      expect(mockLogger.health).toHaveBeenCalledWith(
        'service',
        'healthy',
        { uptime: 3600 }
      );
  ...
  describe('Alert Management', () => {
    ...
    it('should not trigger alerts when conditions are not met', () => {
      ...
      const status = monitoringService.getStatus();
      const mediumAlerts = status.alerts.filter(alert => alert.severity === 'medium');
    ...
    it('should evaluate different alert conditions', () => {
      const testConditions = [
        { condition: 'gt' as const, threshold: 100, value: 150, shouldAlert: true },
        { condition: 'gt' as const, threshold: 100, value: 50, shouldAlert: false },
        { condition: 'lt' as const, threshold: 10, value: 5, shouldAlert: true },
        { condition: 'lt' as const, threshold: 10, value: 15, shouldAlert: false },
        { condition: 'eq' as const, threshold: 100, value: 100, shouldAlert: true },
        { condition: 'eq' as const, threshold: 100, value: 99, shouldAlert: false },
        { condition: 'ne' as const, threshold: 100, value: 99, shouldAlert: true },
        { condition: 'ne' as const, threshold: 100, value: 100, shouldAlert: false }
      ];
      ...
      testConditions.forEach(({ condition, threshold, value, shouldAlert }) => {
        ...
        const status = monitoringService.getStatus();
        const alerts = status.alerts.filter(alert => alert.ruleId === rule.id);
    ...
    it('should send Slack alerts', () => {
      const rule: AlertRule = {
        ...
        actions: [
          {
            type: 'slack',
            config: { webhook: 'https://hooks.slack.com/test', channel: '#alerts' }
          }
        ]
      };
  ...
  describe('Decorators', () => {
    ...
    describe('healthCheck decorator', () => {
      it('should run health checks for decorated methods', async () => {
        ...
        expect(result).toEqual({ status: 'healthy', message: 'Component is healthy' });
      ...
      it('should handle method errors in health check decorator', async () => {
        ...
        await expect(service.failingMethod()).rejects.toThrow('Health check failed');
  ...
  describe('Error Handling', () => {
    it('should handle invalid metric values', () => {
      expect(() => {
        monitoringService.gauge('test.metric', NaN);
      }).not.toThrow();

      expect(() => {
        monitoringService.gauge('test.metric', Infinity);
      }).not.toThrow();
    });
    ...
    it('should handle health check function errors', () => {
      ...
      expect(async () => {
        await monitoringService.runHealthCheck('test.component', invalidCheckFn);
      }).not.toThrow();
    });
  });
  ...
  describe('Concurrent Operations', () => {
    ...
    it('should handle concurrent alert evaluations', async () => {
      ...
      for (let i = 0; i < numOperations; i++) {
        promises.push(
          monitoringService.gauge('concurrent.test', i)
        );
      }
      ...
      const status = monitoringService.getStatus();
      const alerts = status.alerts.filter(alert => alert.ruleId === 'concurrent.alert');
