...
describe("Rate Limiting - Comprehensive Coverage", () => {
	...
	describe("Complex Rate Limiting Scenarios", () => {
		it("should handle sliding window expiration correctly", async () => {
			...
			redisConfig.initializeRedis = jest
				.fn()
				.mockRejectedValue(new Error("Redis unavailable"));
			redisConfig.initializeModernRedis = jest
				.fn()
				.mockRejectedValue(new Error("Redis unavailable"));
			...
			try {
				...
				const result4 = await rateLimitMiddleware(
					mockRequest as NextApiRequest,
					{
						...options,
						keyGenerator: () => "test-key",
				...
				console.log(
					`Request 4: allowed=${result4.allowed}, remaining=${result4.remaining}`,
				);
				...
				const result5 = await rateLimitMiddleware(
					mockRequest as NextApiRequest,
					{
						...options,
						keyGenerator: () => "test-key",
		...
		it("should handle different key generators correctly", async () => {
			const customKeyGenerators = [
				...
				(req: NextApiRequest) =>
					`combined:${req.headers["x-forwarded-for"]}:${req.body?.userId}`,
			];
			...
			for (const keyGenerator of customKeyGenerators) {
				...
				const result1 = await rateLimitMiddleware(
					requestWithBody as NextApiRequest,
					{
						...options,
						keyGenerator,
					},
				...
				const result2 = await rateLimitMiddleware(
					requestWithBody as NextApiRequest,
					{
						...options,
						keyGenerator,
				...
				const result3 = await rateLimitMiddleware(
					requestWithBody as NextApiRequest,
					{
						...options,
						keyGenerator,
	...
	describe("Edge Cases and Error Handling", () => {
		...
		it("should handle zero and negative limits", async () => {
			...
			const zeroResult = await rateLimitMiddleware(
				mockRequest as NextApiRequest,
				zeroLimitOptions,
			);
			const negativeResult = await rateLimitMiddleware(
				mockRequest as NextApiRequest,
				negativeLimitOptions,
			);
	...
	describe("Specialized Rate Limiter Complex Scenarios", () => {
		...
		it("should handle user rate limiting with fallback to IP", async () => {
			...
			// Test with userId
			const resultWithUser = await userRateLimit(
				userRequest as NextApiRequest,
				{
					userId: "user123",
					windowMs: 60000,
					maxRequests: 3,
			...
			const resultWithoutUser = await userRateLimit(
				mockRequest as NextApiRequest,
				{
					windowMs: 60000,
					maxRequests: 3,
		...
		it("should handle mixed rate limiting scenarios", async () => {
			...
			const webhookResult1 = await webhookRateLimit(
				mockRequest as NextApiRequest,
			);
			...
			const webhookResult2 = await webhookRateLimit(
				mockRequest as NextApiRequest,
			);
	...
	describe("Performance and Scalability", () => {
		it("should handle high-frequency requests efficiently", async () => {
			...
			for (let i = 0; i < 1000; i++) {
				requests.push(
					rateLimitMiddleware(mockRequest as NextApiRequest, options),
				);
			}
		...
		it("should handle distributed rate limiting simulation", async () => {
			...
			for (const ip of ips) {
				...
				for (let i = 0; i < 15; i++) {
					ipResults.push(
						rateLimitMiddleware(ipRequest as NextApiRequest, options),
					);
				}
