...
describe("Rate Limiting System", () => {
	...
	describe("Basic Rate Limiting", () => {
		...
		test("should block requests exceeding limit", async () => {
			...
			// First request
			let result = await rateLimitMiddleware(
			...
			// Second request
			result = await rateLimitMiddleware(
				mockRequest as NextApiRequest,
				options,
			);
			expect(result.allowed).toBe(true);
	...
	describe("Edge Cases", () => {
		test("should handle missing IP address", async () => {
			...
			const result = await rateLimitMiddleware(
				requestWithoutIP as NextApiRequest,
				{
					windowMs: 60000,
					maxRequests: 5,
				},
		...
		test("should handle x-forwarded-for with multiple IPs", async () => {
			...
			const result = await rateLimitMiddleware(
				requestWithMultipleIPs as NextApiRequest,
				{
					windowMs: 60000,
					maxRequests: 5,
				},
		...
		test("should handle different keys for different users", async () => {
			...
			const result1 = await rateLimitMiddleware(
				user1Request as NextApiRequest,
				options,
			);
			const result2 = await rateLimitMiddleware(
				user2Request as NextApiRequest,
				options,
			);
