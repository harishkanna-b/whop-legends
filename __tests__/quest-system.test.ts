...
describe("Quest System", () => {
	...
	describe("QuestGenerator", () => {
		it("should generate daily quests based on user level", async () => {
			const quests = await questGenerator.generateDailyQuests(
				mockUser,
				mockCompany,
			);
		...
		it("should scale quest difficulty with user level", async () => {
			...
			const beginnerQuests = await questGenerator.generateDailyQuests(
				beginnerUser,
				mockCompany,
			);
			const advancedQuests = await questGenerator.generateDailyQuests(
				advancedUser,
				mockCompany,
			);

			const avgBeginnerDifficulty =
				beginnerQuests.reduce((sum, q) => sum + q.target_value, 0) /
				beginnerQuests.length;
			const avgAdvancedDifficulty =
				advancedQuests.reduce((sum, q) => sum + q.target_value, 0) /
		...
		it("should apply character class multipliers to rewards", async () => {
			...
			const scoutQuests = await questGenerator.generateDailyQuests(
				scoutUser,
				mockCompany,
			);
			const sageQuests = await questGenerator.generateDailyQuests(
				sageUser,
				mockCompany,
			);
	...
	describe("ProgressTracker", () => {
		...
		it("should check for completed quests", async () => {
			const completedQuests = await progressTracker.checkQuestCompletions(
				mockUser.id,
			);
	...
	describe("RewardManager", () => {
		...
		it("should distribute rewards to user", async () => {
			...
			expect(result.total_commission).toBe(
				mockUser.total_commission + reward.commission,
			);
		});
	...
	describe("Integration Tests", () => {
		it("should handle complete quest lifecycle", async () => {
			// Generate quest
			const quests = await QuestEngine.generateUserQuests(
				mockUser.id,
				mockCompany.id,
				"daily",
			);
			...
			// Update progress
			const updatedQuest = await QuestEngine.updateQuestProgress(
				mockUser.id,
				questId,
				5,
			);
