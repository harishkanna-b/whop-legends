...
import { RateLimiters, RateLimiter } from '../../../lib/security/rate-limit';
...
describe('Security Integration', () => {
  describe('API Route Security', () => {
    ...
    it('should validate all API route parameters', () => {
      ...
      const validationResult = SecurityValidator.validateQuestParams(questParams);
      ...
      const leaderboardValidation = SecurityValidator.validateLeaderboardParams(leaderboardParams);
  ...
  describe('Webhook Security', () => {
    it('should validate webhook payload structure', () => {
      ...
      const userIdValidation = SecurityValidator.validateUserId(invalidPayload.data.user_id);
      expect(userIdValidation.isValid).toBe(false);

      // Test numeric validation
      const amountValidation = SecurityValidator.validate(invalidPayload.data.final_amount, {
        type: 'number',
        min: 0
      });
    ...
    it('should handle rapid validation requests', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        validationPromises.push(
          SecurityValidator.validateUserId(`123e4567-e89b-12d3-a456-426614174${i.toString().padStart(3, '0')}`)
        );
      }
