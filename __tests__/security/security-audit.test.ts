...
describe("SecurityAuditor", () => {
	...
	describe("Security Checks", () => {
		...
		it("should detect input validation issues", async () => {
			...
			const inputValidationMetric = result.metrics.find(
				(m) => m.name === "Input Validation",
			);
...
describe("SecurityValidator", () => {
	describe("Input Validation", () => {
		...
		it("should validate quest parameters correctly", () => {
			...
			const invalidResult =
				SecurityValidator.validateQuestParams(invalidParams);
		...
		it("should detect suspicious progress updates", () => {
			...
			const reasonableProgress = SecurityValidator.validateProgressUpdate(
				userId,
				questId,
				100,
			);
			const suspiciousProgress = SecurityValidator.validateProgressUpdate(
				userId,
				questId,
				10000,
			);
		...
		it("should validate leaderboard parameters", () => {
			...
			const validResult =
				SecurityValidator.validateLeaderboardParams(validParams);
			const invalidResult =
				SecurityValidator.validateLeaderboardParams(invalidParams);
		...
		it("should validate analytics parameters with date ranges", () => {
			...
			const validResult =
				SecurityValidator.validateAnalyticsParams(validParams);
			const invalidResult =
				SecurityValidator.validateAnalyticsParams(invalidParams);
		...
		it("should validate rate limiting parameters", () => {
			const validResult = SecurityValidator.validateRateLimit(
				"user-123",
				"api_request",
				100,
				60000,
			);
			const invalidResult = SecurityValidator.validateRateLimit(
				"",
				"api_request",
				-1,
				0,
		...
		it("should test all data types in generic validation", () => {
			...
			const stringResult = SecurityValidator.validate("true", {
				type: "boolean",
			});
			...
			const objectResult = SecurityValidator.validate(
				{ key: "value" },
				{ type: "object" },
			);
			const arrayResult = SecurityValidator.validate([1, 2, 3], {
				type: "object",
			});
			...
			const validArray = SecurityValidator.validate([1, 2, 3], {
				type: "array",
			});
			const invalidArray = SecurityValidator.validate("not an array", {
				type: "array",
			});
		...
		it("should test custom validation rules", () => {
			...
			const invalidResult = SecurityValidator.validate(
	...
	describe("Error Handling", () => {
		it("should handle null/undefined inputs gracefully", () => {
			...
			const undefinedResult = SecurityValidator.validateUserId(
				undefined as any,
			);
...
describe("Security Integration Tests", () => {
	it("should work with existing validation system", () => {
		...
		const questValidation = SecurityValidator.validateQuestParams(
			testData.questParams,
		);
