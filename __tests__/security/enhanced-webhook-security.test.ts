...
import { validateWebhookRequest, WebhookSignatureError } from '@/lib/webhooks/verify-signature';
...
describe('Enhanced Webhook Security Tests', () => {
  ...
  describe('Webhook Rate Limiting Security', () => {
    it('should handle webhook burst attempts', async () => {
      ...
      const successCount = statusCodes.filter(code => code === 200 || code === 400).length;
  ...
  describe('Webhook Data Validation Security', () => {
    ...
    it('should reject malformed referral data', () => {
      const invalidReferrals = [
        ...
        // Invalid data types
        { event: 'referral.created', data: { id: 123, userId: 'user', amount: 'not-a-number' } },
        // Negative amounts
        { event: 'referral.created', data: { id: 'test', userId: 'user', amount: -100 } },
        // Excessive amounts
        { event: 'referral.created', data: { id: 'test', userId: 'user', amount: 999999999 } },
      ];
