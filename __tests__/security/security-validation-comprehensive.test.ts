...
describe("SecurityValidator - Comprehensive Coverage", () => {
	describe("Missing Data Type Testing", () => {
		it("should validate all numeric types comprehensively", () => {
			...
			expect(SecurityValidator.validate(Math.PI, numberRule).isValid).toBe(
				true,
			);
			expect(SecurityValidator.validate(-0, numberRule).isValid).toBe(true);
			...
			expect(SecurityValidator.validate(Number.NaN, numberRule).isValid).toBe(
				false,
			);
		});
		...
		it("should validate string types with comprehensive patterns", () => {
			...
			expect(SecurityValidator.validate("a@b.", emailRule).isValid).toBe(false); // Too short (4 chars) and missing TLD
			expect(
	...
	describe("Edge Cases and Boundary Conditions", () => {
		...
		it("should handle nested object validation scenarios", () => {
			...
			expect(SecurityValidator.validate(/test/, nestedRule).isValid).toBe(true);
			expect(
