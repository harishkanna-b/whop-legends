...
jest.mock('@/lib/supabase', () => ({
  supabase: {
    from: jest.fn(() => ({
      ...
      insert: jest.fn().mockResolvedValue({ data: [{ id: 'test-achievement' }], error: null }),
      update: jest.fn().mockResolvedValue({ data: { progress: 1, unlocked_at: new Date().toISOString() }, error: null }),
        ...
        data: [
          {
            ...
            achievement: {
              ...
              icon: '🎯',
              category: { id: 'referral', name: 'Referral Mastery', color: '#3B82F6', icon: '🎯' },
              rarity: { id: 'common', name: 'Common', color: '#6B7280', multiplier: 1, icon: '⭐' },
              points: 50,
              requirements: [{ type: 'referrals', target: 1, operator: 'greater_equal', description: 'Make 1 referral' }],
...
describe('AchievementManager', () => {
  ...
  describe('Default Achievements', () => {
    test('should have default achievements defined', () => {
      ...
      expect(DEFAULT_ACHIEVEMENTS.map(a => a.id)).toContain('referral_master')
    ...
    test('achievements should have valid requirements', () => {
      DEFAULT_ACHIEVEMENTS.forEach(achievement => {
        ...
        achievement.requirements.forEach(req => {
          ...
          expect(['equals', 'greater_than', 'less_than', 'greater_equal', 'less_equal']).toContain(req.operator)
        })
      })
    })
  })
  ...
  describe('Achievement Operations', () => {
    ...
    test('should get achievement progress', async () => {
      const progress = await achievementManager.getAchievementProgress(testUserId)
    ...
    test('should get recent achievements', async () => {
      const recent = await achievementManager.getRecentAchievements(testUserId, 5)
  ...
  describe('Progress Updates', () => {
    ...
    test('should handle invalid event type', async () => {
      await expect(
        achievementManager.updateProgress(testUserId, 'invalid_event' as any, 1)
      ).rejects.toThrow()
    })
  })
  ...
  describe('Requirement Checking', () => {
    test('should check equality requirement', () => {
      ...
      const requirement = { type: 'level', target: 5, operator: 'equals' as const }
    ...
    test('should check greater than requirement', () => {
      const manager = (achievementManager as any)
      const requirement = { type: 'level', target: 5, operator: 'greater_than' as const }
    ...
    test('should check less than requirement', () => {
      const manager = (achievementManager as any)
      const requirement = { type: 'level', target: 5, operator: 'less_than' as const }
    ...
    test('should check greater than or equal requirement', () => {
      const manager = (achievementManager as any)
      const requirement = { type: 'level', target: 5, operator: 'greater_equal' as const }
    ...
    test('should check less than or equal requirement', () => {
      const manager = (achievementManager as any)
      const requirement = { type: 'level', target: 5, operator: 'less_equal' as const }
  ...
  describe('Achievement Progression Logic', () => {
    test('should correctly calculate progress percentage', async () => {
      ...
      const progressItem = progress.find(p => p.achievementId === 'first_referral')
      if (progressItem) {
  ...
  describe('Error Handling', () => {
    ...
    test('should handle missing user achievements', async () => {
      ...
      const progress = await achievementManager.getAchievementProgress('nonexistent-user')
      expect(progress).toBeInstanceOf(Array)
    })
