import { CircuitBreaker, CircuitBreakerConfig, CircuitBreakerRegistry, circuitBreaker, defaultCircuitBreakerConfigs } from '@/lib/circuit-breaker/circuit-breaker';
...
describe('Circuit Breaker - Comprehensive Coverage', () => {
  ...
  describe('Expected Exception Filtering', () => {
    it('should count expected exceptions towards failure threshold', async () => {
      const expectedOperation = jest.fn().mockRejectedValue(new Error('test error'));
    ...
    it('should not count unexpected exceptions towards failure threshold', async () => {
      const unexpectedOperation = jest.fn().mockRejectedValue(new Error('unexpected error'));

      // This error should not be counted
      await expect(circuitBreaker.execute(unexpectedOperation)).rejects.toThrow();
  ...
  describe('Error Handling', () => {
    it('should handle synchronous errors in operations', async () => {
      ...
      await expect(circuitBreaker.execute(syncErrorOperation)).rejects.toThrow('sync error');
    });

    it('should handle non-Error exceptions', async () => {
      const nonErrorOperation = jest.fn().mockRejectedValue('string error');

      await expect(circuitBreaker.execute(nonErrorOperation)).rejects.toThrow('string error');
    });
