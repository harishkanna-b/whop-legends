import { ErrorManager, SystemError, CircuitBreaker, errorManager } from '@/lib/error-handling/error-manager';
  ...
  describe('ErrorManager', () => {
    ...
    describe('convenience methods', () => {
      it('should execute database operation with retry', async () => {
        ...
        const result = await errorManager.withDatabaseRetry(
          operation,
          { operation: 'query' }
        );
      ...
      it('should execute external API operation with retry', async () => {
        ...
        const result = await errorManager.withExternalApiRetry(
          operation,
          { operation: 'call' }
        );
      ...
      it('should execute payment operation with retry', async () => {
        ...
        const result = await errorManager.withPaymentRetry(
          operation,
          { operation: 'process' }
        );
    ...
    describe('error metrics', () => {
      it('should track error metrics', () => {
        const error1 = new SystemError(
          'Error 1',
          'ERROR_1',
          'low',
          { component: 'Component1', operation: 'op1' }
        );

        const error2 = new SystemError(
          'Error 2',
          'ERROR_2',
          'high',
          { component: 'Component2', operation: 'op2' }
        );
      ...
      it('should clean up old metrics', () => {
        ...
        const oldHourKey = Math.floor((Date.now() - 25 * 60 * 60 * 1000) / (60 * 60 * 1000));
    ...
    describe('error severity determination', () => {
      it('should determine correct severity for different error types', () => {
        ...
        expect((errorManager as any).determineSeverity(timeoutError)).toBe('high');

        // Test database errors
        const dbError = { code: 'DATABASE_ERROR' };
        expect((errorManager as any).determineSeverity(dbError)).toBe('critical');

        // Test HTTP errors
        const serverError = { status: 500 };
        expect((errorManager as any).determineSeverity(serverError)).toBe('high');

        const clientError = { status: 400 };
        expect((errorManager as any).determineSeverity(clientError)).toBe('medium');

        // Test unknown errors
        const unknownError = {};
        expect((errorManager as any).determineSeverity(unknownError)).toBe('low');
      });
    });
    ...
    describe('retryable error detection', () => {
      it('should correctly identify retryable errors', () => {
        ...
        expect((errorManager as any).isRetryable(explicitlyRetryable)).toBe(true);
