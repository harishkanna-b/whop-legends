	...
	supabase: {
		from: jest.fn(() => ({
			...
			single: jest
				.fn()
				.mockResolvedValue({
					data: { id: "user-123", level: 1, totalXP: 0 },
					error: null,
				}),
			update: jest
				.fn()
				.mockResolvedValue({ data: { level: 2, totalXP: 100 }, error: null }),
		})),
		auth: {
			getUser: jest
				.fn()
...
describe("Webhook End-to-End Integration", () => {
		...
		test("should process different webhook types correctly", async () => {
			...
			for (const type of webhookTypes) {
				...
				const response = await fetch(
					"http://localhost:3001/api/webhooks/whop",
					{
						method: "POST",
						headers: {
							"Content-Type": "application/json",
	...
	describe("Performance and Scalability", () => {
		test("should handle concurrent webhook processing", async () => {
			...
			console.log(
				`Processed ${concurrentRequests} concurrent webhooks in ${processingTime}ms`,
			);
		});
		...
		test("should maintain performance under sustained load", async () => {
			...
			console.log(
				`Sustained load: ${requestsPerSecond.toFixed(2)} RPS, avg response: ${avgResponseTime.toFixed(2)}ms`,
			);
		});
	});
});
