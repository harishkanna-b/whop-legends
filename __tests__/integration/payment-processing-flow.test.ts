...
describe('Payment Processing Flow Integration Tests', () => {
  beforeEach(() => {
    ...
    (supabaseService.from as jest.Mock).mockImplementation(mockSupabaseService.from);
    (supabaseService.raw as jest.Mock).mockImplementation(mockSupabaseService.raw);
  ...
  describe('Complete Payment-to-Referral Flow', () => {
    it('should process complete payment with referrer successfully', async () => {
      ...
      await processDirectPayment(paymentData.userId, paymentData.amount, paymentData.currency);
