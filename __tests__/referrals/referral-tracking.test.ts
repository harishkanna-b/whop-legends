...
describe('ReferralManager - Core Functionality', () => {
  ...
  test('should handle database connection errors gracefully', async () => {
    ...
    const analytics = await referralManager.getReferralAnalytics('test-user', '30d')
  ...
  test('should handle empty results gracefully', async () => {
    const referrals = await referralManager.getUserReferrals('nonexistent-user')
    expect(Array.isArray(referrals)).toBe(true)
    expect(referrals).toHaveLength(0)

    const campaigns = await referralManager.getUserCampaigns('nonexistent-user')
    expect(Array.isArray(campaigns)).toBe(true)
