...
describe('QuestEngine - Core Functionality', () => {
  ...
  describe('Quest Generation', () => {
    ...
    it('should generate weekly quests with higher rewards and complexity', async () => {
      const weeklyConfig = { ...mockQuestConfig, quest_type: 'weekly' as const };
    ...
    it('should generate monthly quests with prestige rewards', async () => {
      const monthlyConfig = { ...mockQuestConfig, quest_type: 'monthly' as const };
    ...
    it('should fallback to default quests when templates are not available', async () => {
      (supabaseService.from as jest.Mock).mockReturnValue({
        select: jest.fn().mockReturnValue({
          eq: jest.fn().mockReturnValue({
            single: jest.fn().mockResolvedValue({ data: null, error: 'No templates found' }),
          }),
        }),
      });
    ...
    it('should validate user ID before generating quests', async () => {
      ...
      await expect(QuestEngine.generateDailyQuests(mockQuestConfig)).rejects.toThrow(
        'Invalid user ID'
      );
    });
  ...
  describe('User Quest Management', () => {
    it('should retrieve user active quests', async () => {
      ...
      (supabaseService.from as jest.Mock).mockReturnValue({
        select: jest.fn().mockReturnValue({
          eq: jest.fn().mockReturnValue({
            eq: jest.fn().mockReturnValue({
              order: jest.fn().mockResolvedValue({ data: mockUserQuests, error: null }),
            }),
          }),
        }),
      });
    ...
    it('should calculate quest progress correctly', async () => {
      ...
      (supabaseService.from as jest.Mock).mockReturnValue({
        select: jest.fn().mockReturnValue({
          eq: jest.fn().mockReturnValue({
            single: jest.fn().mockResolvedValue({ data: mockUserStats, error: null }),
          }),
        }),
      });
  ...
  describe('Quest Progress Updates', () => {
    ...
    it('should validate user ID before updating progress', async () => {
      ...
      await expect(QuestEngine.updateQuestProgress(mockUserId, 'referral', 1)).rejects.toThrow(
        'Invalid user ID'
      );
    });

    it('should validate action type before updating progress', async () => {
      await expect(QuestEngine.updateQuestProgress(mockUserId, 'invalid_action' as any, 1)).rejects.toThrow(
        'Invalid action type'
      );
    });
    ...
    it('should validate progress value before updating', async () => {
      ...
      await expect(QuestEngine.updateQuestProgress(mockUserId, 'referral', 1001)).rejects.toThrow(
        'Progress value must be between 0 and 1000'
      );
    });
  });
  ...
  describe('Quest Reward Distribution', () => {
    it('should claim quest rewards successfully', async () => {
      ...
      const reward = await QuestEngine.claimQuestRewards(mockUserId, 'user-quest-1');
  ...
  describe('Quest Statistics', () => {
    it('should calculate user quest statistics', async () => {
      ...
      (supabaseService.from as jest.Mock).mockReturnValue({
        select: jest.fn().mockReturnValue({
          eq: jest.fn().mockReturnValue({
            eq: jest.fn().mockResolvedValue({ data: mockUserQuests, error: null }),
          }),
        }),
      });
    ...
    it('should return default stats when no quests found', async () => {
      (supabaseService.from as jest.Mock).mockReturnValue({
        select: jest.fn().mockReturnValue({
          eq: jest.fn().mockReturnValue({
            eq: jest.fn().mockResolvedValue({ data: null, error: 'No quests found' }),
          }),
        }),
      });
  ...
  describe('Scheduled Quest Generation', () => {
    it('should generate daily quests at midnight UTC', async () => {
      ...
      const mockUsers = [
        { id: mockUserId, company_id: mockCompanyId, level: 5, character_class: 'scout' },
      ];

      (supabaseService.from as jest.Mock).mockReturnValue({
        select: jest.fn().mockReturnValue({
          eq: jest.fn().mockReturnValue({
            single: jest.fn().mockResolvedValue({ data: null, error: 'No templates' }),
          }),
        }),
      });
