...
import { QuestProgress, type QuestStatus } from "@/lib/types/quest-types";
...
describe("ProgressTracker - Quest Progress Management", () => {
	...
	describe("Progress Calculation", () => {
		it("should calculate progress percentage correctly", () => {
			...
			const progressComplete = ProgressTracker.calculateProgressPercentage(
				10,
				10,
			);
		...
		it("should handle edge cases in progress calculation", () => {
			...
			const progressZeroTarget = ProgressTracker.calculateProgressPercentage(
				5,
				0,
			);
			...
			const progressNegative = ProgressTracker.calculateProgressPercentage(
				-1,
				10,
			);
			...
			const progressExceeded = ProgressTracker.calculateProgressPercentage(
				15,
				10,
			);
		...
		it("should format time remaining for different durations", () => {
			...
			// Test days
			const daysRemaining = ProgressTracker.calculateTimeRemaining(
				now + 3 * 24 * 60 * 60 * 1000,
			);
			expect(daysRemaining).toBe("3d 0h");

			// Test hours only
			const hoursRemaining = ProgressTracker.calculateTimeRemaining(
				now + 5 * 60 * 60 * 1000,
			);
			expect(hoursRemaining).toBe("5h 0m");

			// Test minutes only
			const minutesRemaining = ProgressTracker.calculateTimeRemaining(
				now + 30 * 60 * 1000,
			);
	...
	describe("Progress Updates", () => {
		it("should validate user ID before updating progress", async () => {
			...
			await expect(
				ProgressTracker.updateQuestProgress(
					mockUserId,
					mockQuestId,
					"referrals",
					1,
	...
	describe("Progress Validation", () => {
		it("should validate progress increments", () => {
			...
			validIncrements.forEach((increment) => {
				expect(() =>
					ProgressTracker.validateProgressIncrement(increment),
				).not.toThrow();
			});
		...
		it("should validate action types", () => {
			...
			invalidActions.forEach((action) => {
				expect(() =>
					ProgressTracker.validateActionType(action as any),
				).toThrow();
			});
		});
		...
		it("should validate quest state transitions", () => {
			// Valid transitions
			expect(() =>
				ProgressTracker.validateStateTransition("active", "completed"),
			).not.toThrow();
			expect(() =>
				ProgressTracker.validateStateTransition("active", "failed"),
			).not.toThrow();
			expect(() =>
				ProgressTracker.validateStateTransition("active", "expired"),
			).not.toThrow();

			// Invalid transitions
			expect(() =>
				ProgressTracker.validateStateTransition("completed", "active"),
			).toThrow();
			expect(() =>
				ProgressTracker.validateStateTransition("failed", "active"),
			).toThrow();
			expect(() =>
				ProgressTracker.validateStateTransition("expired", "active"),
			).toThrow();
		});
	});
	...
	describe("Real-time Progress Updates", () => {
		it("should broadcast progress updates", async () => {
			...
			await ProgressTracker.broadcastProgressUpdate(
				progressUpdate,
				mockBroadcast,
			);
	...
	describe("Progress Analytics", () => {
		it("should calculate completion rate analytics", () => {
			...
			const analytics =
				ProgressTracker.calculateCompletionAnalytics(userProgressData);
		...
		it("should calculate progress velocity", () => {
			...
			const velocity =
				ProgressTracker.calculateProgressVelocity(progressTimeline);
		...
		it("should predict quest completion time", () => {
			...
			const prediction = ProgressTracker.predictCompletionTime(
				progressTimeline,
				20,
			);
				...
				select: jest.fn().mockReturnValue({
					eq: jest.fn().mockReturnValue({
						eq: jest.fn().mockReturnValue({
							single: jest
								.fn()
								.mockResolvedValue({ data: null, error: "Quest not found" }),
						}),
					}),
				}),
			});
