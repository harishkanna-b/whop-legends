...
import type { Quest, QuestReward, UserQuest } from "@/lib/types/quest-types";
...
describe("RewardManager - Quest Reward Distribution", () => {
	...
	describe("Reward Calculation", () => {
		it("should calculate base rewards correctly", () => {
			...
			expect(rewards.commission).toBe(10.0);
		});
		...
		it("should apply character class multipliers correctly", () => {
			...
			const scoutRewards = RewardManager.applyCharacterClassMultiplier(
				baseRewards,
				"scout",
			);
			expect(scoutRewards.xp).toBe(110);
			expect(scoutRewards.commission).toBe(10.0);

			// Sage: 1.0x XP, 1.2x commission
			const sageRewards = RewardManager.applyCharacterClassMultiplier(
				baseRewards,
				"sage",
			);
			expect(sageRewards.xp).toBe(100);
			expect(sageRewards.commission).toBe(12.0);

			// Champion: 1.15x XP, 1.1x commission
			const championRewards = RewardManager.applyCharacterClassMultiplier(
				baseRewards,
				"champion",
			);
			expect(championRewards.xp).toBe(115);
			expect(championRewards.commission).toBe(11.0);
		});
		...
		it("should apply difficulty multipliers", () => {
			...
			const easyRewards = RewardManager.applyDifficultyMultiplier(
				baseRewards,
				"easy",
			);
			const mediumRewards = RewardManager.applyDifficultyMultiplier(
				baseRewards,
				"medium",
			);
			const hardRewards = RewardManager.applyDifficultyMultiplier(
				baseRewards,
				"hard",
			);
			const epicRewards = RewardManager.applyDifficultyMultiplier(
				baseRewards,
				"epic",
			);
		...
		it("should apply quest type multipliers", () => {
			...
			const dailyRewards = RewardManager.applyQuestTypeMultiplier(
				baseRewards,
				"daily",
			);
			const weeklyRewards = RewardManager.applyQuestTypeMultiplier(
				baseRewards,
				"weekly",
			);
			const monthlyRewards = RewardManager.applyQuestTypeMultiplier(
				baseRewards,
				"monthly",
			);
		...
		it("should round monetary rewards correctly", () => {
			...
			const roundedRewards =
				RewardManager.roundMonetaryRewards(unroundedRewards);
	...
	describe("Reward Distribution", () => {
		it("should distribute rewards to user successfully", async () => {
				...
				select: jest.fn().mockReturnValue({
					eq: jest.fn().mockReturnValue({
						eq: jest.fn().mockReturnValue({
							single: jest
								.fn()
								.mockResolvedValue({ data: userQuest, error: null }),
						}),
					}),
				}),
			});
			...
			const result = await RewardManager.distributeQuestRewards(
				userQuest,
				quest,
			);
	...
	describe("Reward History and Tracking", () => {
		...
		it("should calculate reward distribution analytics", async () => {
			...
			const analytics =
				await RewardManager.getRewardDistributionAnalytics(companyId);
	...
	describe("Special Rewards and Bonuses", () => {
		it("should calculate first-completion bonuses", () => {
			...
			const firstCompletionBonus = RewardManager.calculateFirstCompletionBonus(
				baseRewards,
				true,
			);
			const noBonus = RewardManager.calculateFirstCompletionBonus(
				baseRewards,
				false,
			);

			expect(firstCompletionBonus.xp).toBeGreaterThan(noBonus.xp);
			expect(firstCompletionBonus.commission).toBeGreaterThan(
				noBonus.commission,
			);
		});
		...
		it("should calculate speed completion bonuses", () => {
			...
			const fastBonus = RewardManager.calculateSpeedBonus(
				baseRewards,
				startTime,
				endTime,
			);
		...
		it("should calculate perfect completion bonuses", () => {
			...
			const perfectBonus = RewardManager.calculatePerfectCompletionBonus(
				baseRewards,
				true,
			);
			const imperfectBonus = RewardManager.calculatePerfectCompletionBonus(
				baseRewards,
				false,
			);
	...
	describe("Reward Validation", () => {
		...
		it("should validate reward distribution limits", () => {
			...
			expect(() =>
				RewardManager.validateRewardLimits(reasonableRewards),
			).not.toThrow();
			expect(() =>
				RewardManager.validateRewardLimits(excessiveRewards),
			).toThrow();
		});
	});
	...
	describe("Reward System Configuration", () => {
		...
		it("should validate reward system configuration", () => {
			const validConfig = {
				...
				max_daily_rewards: { xp: 1000, commission: 100.0 },
			};
			...
			expect(() =>
				RewardManager.validateRewardConfiguration(validConfig),
			).not.toThrow();
			expect(() =>
				RewardManager.validateRewardConfiguration(invalidConfig),
			).toThrow();
		});
	});
});
