...
import { QuestTemplate, QuestDifficulty, QuestType } from '@/lib/types/quest-types';
...
describe('QuestGenerator - Quest Generation Logic', () => {
  ...
  const mockSecurityValidator = require('@/lib/security/validation').SecurityValidator;
  ...
  describe('Template-based Quest Generation', () => {
    ...
    it('should apply character class multipliers to rewards', () => {
      ...
      const quests = QuestGenerator.generateQuestsFromTemplates(config, mockTemplates);
  ...
  describe('Dynamic Quest Generation', () => {
    it('should generate quests based on user progression', () => {
      ...
      const quests = QuestGenerator.generateDynamicQuests(userStats, mockCompanyId);

      expect(quests).toHaveLength(3);

      // Should generate harder quests for higher level users
      const hasHardQuest = quests.some(q => q.difficulty === 'hard' || q.difficulty === 'epic');
      expect(hasHardQuest).toBe(true);
    });

    it('should balance quest rewards based on difficulty', () => {
      const easyQuest = QuestGenerator.createBalancedQuest('easy', mockCompanyId);
      const mediumQuest = QuestGenerator.createBalancedQuest('medium', mockCompanyId);
      const hardQuest = QuestGenerator.createBalancedQuest('hard', mockCompanyId);
      ...
      expect(easyQuest.reward_commission).toBeLessThan(mediumQuest.reward_commission);
      expect(mediumQuest.reward_commission).toBeLessThan(hardQuest.reward_commission);
    });
  ...
  describe('Quest Scaling Logic', () => {
    ...
    it('should apply character class multipliers correctly', () => {
      const baseReward = { xp: 100, commission: 10.00 };

      const scoutReward = QuestGenerator.applyCharacterClassMultiplier(baseReward, 'scout');
      const sageReward = QuestGenerator.applyCharacterClassMultiplier(baseReward, 'sage');
      const championReward = QuestGenerator.applyCharacterClassMultiplier(baseReward, 'champion');

      // Scout: 1.1x XP, 1.1x commission
      expect(scoutReward.xp).toBe(110);
      expect(scoutReward.commission).toBe(11.00);

      // Sage: 1.0x XP, 1.2x commission
      expect(sageReward.xp).toBe(100);
      expect(sageReward.commission).toBe(12.00);

      // Champion: 1.15x XP, 1.1x commission
      expect(championReward.xp).toBe(115);
      expect(championReward.commission).toBe(11.00);
    });
  });
  ...
  describe('Quest Diversity', () => {
    it('should generate diverse quest objectives', () => {
      ...
      const quest2 = QuestGenerator.createBalancedQuest('medium', mockCompanyId);
    ...
    it('should ensure quests have meaningful time limits', () => {
      const dailyQuest = QuestGenerator.createBalancedQuest('easy', mockCompanyId, 'daily');
      const weeklyQuest = QuestGenerator.createBalancedQuest('medium', mockCompanyId, 'weekly');
      const monthlyQuest = QuestGenerator.createBalancedQuest('hard', mockCompanyId, 'monthly');
      ...
      const dailyHours = (dailyEnd.getTime() - now.getTime()) / (1000 * 60 * 60);
      const weeklyHours = (weeklyEnd.getTime() - now.getTime()) / (1000 * 60 * 60);
      const monthlyHours = (monthlyEnd.getTime() - now.getTime()) / (1000 * 60 * 60);
