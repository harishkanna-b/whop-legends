...
describe('Character Classes System', () => {
  ...
  describe('Class Recommendation System', () => {
    it('should recommend sage for high-value patterns', async () => {
      ...
      const mockReferrals = [
        { value: 150, status: 'completed', created_at: new Date().toISOString() },
        { value: 200, status: 'completed', created_at: new Date().toISOString() },
      ];

      // Mock the database call
      jest.spyOn(CharacterClassManager as any, 'analyzeReferralPattern').mockReturnValue({
      ...
      const recommendation = await CharacterClassManager.recommendClass('user123');

      expect(recommendation.classId).toBe('sage');
      expect(recommendation.confidence).toBeGreaterThan(0.7);
      expect(recommendation.reasons).toContain('High-value referral pattern detected');
    });
    ...
    it('should recommend champion for volume patterns', async () => {
      ...
      expect(recommendation.reasons).toContain('Strong referral volume detected');
    });
    ...
    it('should recommend scout for consistent patterns', async () => {
      ...
      expect(recommendation.reasons).toContain('Consistent referral pattern detected');
    });
  ...
  describe('Pattern Analysis', () => {
    it('should analyze referral patterns correctly', () => {
      const referrals = [
        { value: 120, status: 'completed', created_at: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString() },
        { value: 150, status: 'completed', created_at: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString() },
        { value: 180, status: 'completed', created_at: new Date(Date.now() - 8 * 24 * 60 * 60 * 1000).toISOString() },
      ];
      ...
      const analyzeReferralPattern = (CharacterClassManager as any).analyzeReferralPattern.bind(CharacterClassManager);
    ...
    it('should calculate frequency correctly', () => {
      ...
      const referrals = [
        ...
        { value: 60, status: 'completed', created_at: oneWeekAgo.toISOString() },
      ];
