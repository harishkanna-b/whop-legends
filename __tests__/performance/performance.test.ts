...
describe('Performance Tests', () => {
  describe('Rate Limiting Performance', () => {
    it('should handle 1000 requests in under 100ms', async () => {
      ...
      console.log(`Rate limiting performance: ${requestsPerSecond.toFixed(2)} req/sec`);
  ...
  describe('Webhook Signature Performance', () => {
    it('should verify 1000 signatures in under 50ms', async () => {
      ...
      console.log(`Signature verification performance: ${verificationsPerSecond.toFixed(2)} verifications/sec`);
      console.log(`Total time: ${totalTime.toFixed(2)}ms`);
  ...
  describe('Memory Usage', () => {
    it('should handle memory efficiently under load', () => {
      ...
      console.log(`Memory increase under load: ${(heapIncrease / 1024 / 1024).toFixed(2)} MB`);
  ...
  describe('Database Query Performance', () => {
    it('should handle database queries efficiently', async () => {
      ...
      const mockSelect = supabase.from('users').select('count', { count: 'exact', head: true });
  ...
  describe('NFR Compliance', () => {
    it('should meet Non-Functional Requirements', () => {
      ...
      console.log(`✅ Response Time: ${actualResults.avgResponseTime}ms < ${nfrs.maxResponseTime}ms`);
      console.log(`✅ Memory Usage: ${actualResults.peakMemoryUsage}MB < ${nfrs.maxMemoryUsage}MB`);
      console.log(`✅ Throughput: ${actualResults.throughput} req/sec > ${nfrs.minThroughput} req/sec`);
      console.log(`✅ Error Rate: ${(actualResults.errorRate * 100).toFixed(2)}% < ${(nfrs.maxErrorRate * 100).toFixed(2)}%`);
    });
  });
});