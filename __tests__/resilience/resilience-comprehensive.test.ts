import { ResilienceManager, ResilienceConfig, ResilienceManagerRegistry, defaultResilienceConfigs, resilient } from '@/lib/resilience/resilience-manager';
...
describe('Resilience Manager - Comprehensive Coverage', () => {
  ...
  describe('Basic Resilience Patterns', () => {
    ...
    it('should apply timeout to operations', async () => {
      ...
      await expect(resilienceManager.execute(slowOperation)).rejects.toThrow('Operation timeout');
    });
  ...
  describe('Circuit Breaker Integration', () => {
    it('should include circuit breaker when configured', () => {
      expect(resilienceManager.getCircuitBreaker()).toBeInstanceOf(CircuitBreaker);
    });

    it('should not include circuit breaker when not configured', () => {
      const noCbConfig: ResilienceConfig = { ...config, circuitBreaker: undefined };
    ...
    it('should handle circuit breaker failures gracefully', async () => {
      ...
      await expect(resilienceManager.execute(mockOperation)).rejects.toThrow('Circuit breaker is OPEN');
    });
  });
  ...
  describe('Retry Logic', () => {
    ...
    it('should respect max retry attempts', async () => {
      ...
      await expect(resilienceManager.execute(alwaysFailingOperation)).rejects.toThrow('Always fails');
    ...
    it('should apply exponential backoff with jitter', async () => {
      ...
      await expect(resilienceManager.execute(failingOperation)).rejects.toThrow();
    ...
    it('should not retry when retry is not configured', async () => {
      ...
      const failingOperation = jest.fn().mockRejectedValue(new Error('Fails once'));

      await expect(noRetryManager.execute(failingOperation)).rejects.toThrow('Fails once');
  ...
  describe('Caching', () => {
    ...
    it('should handle cache expiration', async () => {
      const shortCacheConfig: ResilienceConfig = {
        ...config,
        cache: { ttl: 100, keyGenerator: (...args: any[]) => JSON.stringify(args) }
      };
      const shortCacheManager = new ResilienceManager('short-cache', shortCacheConfig);
  ...
  describe('Bulkhead Pattern', () => {
    ...
    it('should reject requests when bulkhead queue is full', async () => {
      const verySlowOperation = jest.fn().mockImplementation(
    ...
    it('should not use bulkhead when not configured', async () => {
      const noBulkheadConfig: ResilienceConfig = { ...config, bulkhead: undefined };
      const noBulkheadManager = new ResilienceManager('no-bulkhead', noBulkheadConfig);
  ...
  describe('Failover Integration', () => {
    it('should include failover manager when configured', () => {
      ...
      const failoverManager = new ResilienceManager('failover-test', failoverConfig);
      expect(failoverManager.getFailoverManager()).toBeInstanceOf(FailoverManager);
    });
  ...
  describe('Metrics Collection', () => {
    it('should collect comprehensive metrics', async () => {
      ...
      try {
        await resilienceManager.execute(jest.fn().mockRejectedValue(new Error('test')));
      } catch (error) {
        // Expected to fail
      }
  ...
  describe('Availability Check', () => {
    ...
    it('should always be available when no circuit breaker', () => {
      const noCbManager = new ResilienceManager('no-cb', { retry: config.retry });
  ...
  describe('Error Handling', () => {
    it('should handle operations that throw non-Error objects', async () => {
      ...
      await expect(resilienceManager.execute(stringErrorOperation)).rejects.toThrow('string error');
    });

    it('should handle operations that throw null/undefined', async () => {
      const nullErrorOperation = jest.fn().mockRejectedValue(null);

      await expect(resilienceManager.execute(nullErrorOperation)).rejects.toThrow();
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle operation timeouts longer than configured timeout', async () => {
      ...
      await expect(timeoutManager.execute(slowOperation)).rejects.toThrow('Operation timeout');
    });
    ...
    it('should handle cache with custom key generator', async () => {
      ...
      const customManager = new ResilienceManager('custom-cache', customCacheConfig);
