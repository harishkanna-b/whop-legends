...
describe("Failover Manager - Comprehensive Coverage", () => {
	...
	describe("Health Check System", () => {
		...
		it("should update provider availability based on health checks", async () => {
			...
			jest
				.spyOn(failoverManager as any, "performHealthCheck")
	...
	describe("Provider Management", () => {
		...
		it("should handle provider metadata updates", () => {
			failoverManager.updateProviderMetadata("primary", {
				region: "us-east-1",
				version: "1.0.0",
			});
			failoverManager.updateProviderMetadata("fallback1", {
				region: "us-west-1",
				version: "1.0.0",
			});
	...
	describe("Configuration Options", () => {
		...
		it("should respect failback threshold", async () => {
			...
			if (primaryStatus) {
				primaryStatus.isAvailable = true;
				primaryStatus.consecutiveSuccesses =
					thresholdConfig.failbackThreshold - 1;
			}
	...
	describe("Health Check Management", () => {
		...
		it("should handle health check failures gracefully", async () => {
			...
			await expect(
				(failoverManager as any).checkAllProviders(),
	...
	describe("Edge Cases", () => {
		...
		it("should handle single fallback provider", () => {
			const singleFallbackConfig = {
				...config,
				fallbackProviders: ["fallback1"],
			};
