...
export const retryWebhookProcessing = async (
  ...
  for (let attempt = 1; attempt <= config.maxRetries; attempt++) {
    try {
      ...
      // Check if we should retry
      if (attempt === config.maxRetries || !isRetryableError(webhookError, config)) {
        break;
      }

      // Calculate delay and wait
      const delay = calculateBackoffDelay(attempt, config);
      console.log(`Webhook ${webhookId} attempt ${attempt} failed, retrying in ${delay}ms...`);
...
export const retryWithRateLimit = async <T>(
  ...
  for (let attempt = 1; attempt <= config.maxRetries; attempt++) {
    try {
      ...
      if (rateError.message.includes('rate_limit') || (error as any)?.status === 429) {
      ...
      if (attempt === config.maxRetries || !isRetryableError(rateError, config)) {
        break;
      }
