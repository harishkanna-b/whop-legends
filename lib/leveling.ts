...
export class LevelingManager {
	 ...
	 */
	static calculateTotalXPToLevel(
		level: number,
		characterClass?: CharacterClass,
	): number {
		let totalXP = 0;
		for (let i = 1; i < level; i++) {
			totalXP += LevelingManager.calculateXPRequired(i, characterClass);
		}
		return totalXP;
	}
	...
	static calculateLevelFromXP(
		...
		while (true) {
			const xpForNextLevel = LevelingManager.calculateXPRequired(
				level + 1,
				characterClass,
			);
	...
	static async getUserProgression(userId: string): Promise<LevelProgression> {
		try {
			...
			const characterClass = userClass
				? CharacterClassManager.getClassById(userClass.classId)
				: null;
			...
			if (error || !userXP) {
				...
				return {
					...
					currentXP: 0,
					totalXP: 0,
					xpToNextLevel: LevelingManager.calculateXPRequired(
						2,
			...
			const xpForCurrentLevel = LevelingManager.calculateTotalXPToLevel(
				currentLevel,
				characterClass || undefined,
			);
			const currentXP = totalXP - xpForCurrentLevel;
			const xpToNextLevel = LevelingManager.calculateXPRequired(
				currentLevel + 1,
				characterClass || undefined,
			);
			...
			const levelHistory: LevelHistory[] = (history || []).map((h: any) => ({
				level: h.level,
				xp: h.xp_at_level,
	...
	static async addXP(
		...
		try {
			...
			const currentProgression =
				await LevelingManager.getUserProgression(userId);
			const userClass = await CharacterClassManager.getUserClass(userId);
			const characterClass = userClass
				? CharacterClassManager.getClassById(userClass.classId)
				: null;
			...
			const newLevel = LevelingManager.calculateLevelFromXP(
				newTotalXP,
				characterClass || undefined,
			);
			...
			if (newLevel > currentProgression.currentLevel) {
				for (
					let level = currentProgression.currentLevel + 1;
					level <= newLevel;
					level++
				) {
					const unlocks = LevelingManager.getLevelUnlocks(
						level,
						characterClass || undefined,
					);
					const message = LevelingManager.generateLevelUpMessage(
						level,
						characterClass || undefined,
					);
					...
					await LevelingManager.recordLevelUp(
						userId,
						level,
						newTotalXP,
						unlocks[0],
			...
			await LevelingManager.updateUserXP(userId, newTotalXP, newLevel);

			// Record XP event
			const xpEvent = await LevelingManager.recordXPEvent(userId, {
				type,
				amount: finalAmount,
				description,
				metadata,
			...
			const newProgression = await LevelingManager.getUserProgression(userId);

			// Send level up notifications if any
			for (const levelUp of levelUps) {
				await LevelingManager.sendLevelUpNotification(levelUp);
			}
	 ...
	 * Update user's XP and level in database
	 */
	private static async updateUserXP(
		userId: string,
		totalXP: number,
		level: number,
	) {
	 ...
	 */
	private static async recordXPEvent(
		userId: string,
		event: Omit<XPEvent, "id" | "userId" | "createdAt">,
	): Promise<XPEvent> {
	 ...
	 * Record level up in history
	 */
	private static async recordLevelUp(
		userId: string,
		level: number,
		xp: number,
		achievement?: string,
	...
	static calculateXPForEvent(
		...
		switch (type) {
			...
			case "achievement":
				...
				return metadata?.rarity === "legendary"
					? 200
	 ...
	 */
	static async getXPHistory(userId: string, limit = 50): Promise<XPEvent[]> {
		try {
			const { data, error } = await supabaseService()
				.from("user_xp_events")
				.select("*")
	 ...
	 */
	static async getLeaderboard(limit = 10, classFilter?: string) {
		try {
