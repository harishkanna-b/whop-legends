...
export class CircuitBreaker {
  ...
  private onSuccess(): void {
    ...
    if (this.state === 'half-open') {
      this.successCount++;
      if (this.successCount >= 3) { // Require 3 consecutive successes to close
        this.state = 'closed';
        this.successCount = 0;
      }
    }
  ...
  private onFailure(error: any): void {
    ...
    const isExpectedError = this.config.expectedException?.some(
      expected => {
...
export class ErrorManager {
  ...
  private errorHandlers: Map<string, (error: SystemError) => Promise<void>> = new Map();
  ...
  async executeWithRetry<T>(
    ...
    if (strategy.fallback) {
      try {
        ...
        this.logError(this.wrapError(fallbackError, {
          ...context,
  ...
  private calculateRetryDelay(strategy: ErrorRecoveryStrategy, attempt: number): number {
  ...
  private updateErrorMetrics(error: SystemError): void {
    ...
    metrics.byComponent[error.context.component] = (metrics.byComponent[error.context.component] || 0) + 1;
    metrics.bySeverity[error.severity]++;

    // Clean up old metrics (keep last 24 hours)
    const twentyFourHoursAgo = Math.floor((now - 24 * 60 * 60 * 1000) / (60 * 60 * 1000));
    for (const [key] of this.errorMetrics) {
      if (parseInt(key) < twentyFourHoursAgo) {
  ...
  private sendToMonitoring(error: SystemError): void {
    ...
    console.log('MONITORING_PAYLOAD:', JSON.stringify(monitoringPayload, null, 2));
  }
  ...
  // Public API
  registerErrorHandler(handlerId: string, handler: (error: SystemError) => Promise<void>): void {
  ...
  getErrorMetrics(): ErrorMetrics {
    ...
    const relevantMetrics = Array.from(this.errorMetrics.entries())
      .filter(([timestamp]) => parseInt(timestamp) * 60 * 60 * 1000 > oneHourAgo)
    ...
    return {
      ...
      errorByType: relevantMetrics.reduce((acc, m) => {
        Object.entries(m.byType).forEach(([type, count]) => {
          acc[type] = (acc[type] || 0) + count;
        });
        return acc;
      }, {} as Record<string, number>),
      errorByComponent: relevantMetrics.reduce((acc, m) => {
