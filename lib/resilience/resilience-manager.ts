import { CircuitBreaker, CircuitBreakerConfig, CircuitBreakerRegistry, defaultCircuitBreakerConfigs } from '../circuit-breaker/circuit-breaker';
import { FailoverManager, FailoverConfig, FailoverManagerRegistry, defaultFailoverConfigs } from '../failover/failover-manager';
...
export class ResilienceManager {
  ...
  private bulkheadQueue: Array<{ resolve: Function; reject: Function; operation: Function }> = [];
  ...
  private initializeComponents(): void {
    ...
    if (this.config.circuitBreaker) {
      ...
      this.circuitBreaker = circuitBreakerRegistry.register(`${this.name}-circuit-breaker`, this.config.circuitBreaker);
    }

    // Initialize failover manager
    if (this.config.failover) {
      const failoverRegistry = FailoverManagerRegistry.getInstance();
      this.failoverManager = failoverRegistry.register(`${this.name}-failover`, this.config.failover);
    }
  }
  ...
  async execute<T>(
    ...
    try {
      ...
      if (this.config.cache && !options?.skipCache && options?.cacheKey) {
        ...
        if (cached !== undefined) {
          monitoring.increment('resilience.cache.hit', 1, { manager: this.name });
        ...
        monitoring.increment('resilience.cache.miss', 1, { manager: this.name });
      }
      ...
      if (this.circuitBreaker) {
        return this.circuitBreaker.execute(async () => {
          ...
          if (this.failoverManager) {
            return this.failoverManager.execute(async (provider: string) => {
  ...
  private async executeWithBulkhead<T>(operation: () => Promise<T>, context: Record<string, any>): Promise<T> {
    ...
    if (this.bulkheadQueue.length >= maxConcurrent * 2) {
      monitoring.increment('resilience.bulkhead.rejected', 1, { manager: this.name });
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        const index = this.bulkheadQueue.findIndex(item => item.reject === reject);
        if (index !== -1) {
  ...
  private processBulkheadQueue(): void {
    while (this.bulkheadQueue.length > 0 && this.activeBulkheadRequests < this.config.bulkhead!.maxConcurrent) {
  ...
  getMetrics(): ResilienceMetrics {
    const averageRetryDelay = this.retryMetrics.totalAttempts > 0
      ? this.retryMetrics.totalDelay / this.retryMetrics.totalAttempts
      : 0;

    const cacheHits = monitoring.getStatus().metrics.names.includes('resilience.cache.hit') ?
      this.retryMetrics.successfulRetries : 0;
    const cacheMisses = monitoring.getStatus().metrics.names.includes('resilience.cache.miss') ?
  ...
  getHealth(): {
    ...
    const failoverHealth = this.failoverManager ? {
      ...
      availableProviders: this.failoverManager.getAllProviders()
