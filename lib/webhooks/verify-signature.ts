...
import { WebhookSignature } from '@/types/whop';
...
export const verifyWebhookSignature = (
  ...
  try {
    ...
    throw new WebhookSignatureError(`Signature verification failed: ${message}`);
  }
};

export const extractSignatureFromHeader = (signatureHeader: string): WebhookSignature => {
...
export const validateWebhookRequest = (
  ...
  try {
    ...
    const isValid = verifyWebhookSignature(payload, signatureHeader, timestampHeader, secret);

    return { isValid, eventType };
  } catch (error) {
    return {
      isValid: false,
      error: error instanceof WebhookSignatureError ? error.message : 'Unknown error'
    };
  }
};