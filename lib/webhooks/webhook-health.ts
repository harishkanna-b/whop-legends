...
export class WebhookHealthMonitor {
  ...
  async getMetrics(): Promise<WebhookHealthMetrics> {
    ...
    try {
      ...
      const { data: webhooks, error: webhookError } = await supabaseService
        .from('webhook_logs')
        .select('status, processing_time_ms, created_at, webhook_id, event_type')
      ...
      const successfulWebhooks = webhooks?.filter(w => w.status === 'success').length || 0;
      ...
      const processingTimes = webhooks?.filter(w => w.processing_time_ms).map(w => w.processing_time_ms) || [];
      ...
      const lastProcessed = webhooks && webhooks.length > 0
        ? webhooks[0].created_at
        : null;
  ...
  private async getRecentErrors(limit: number = 10): Promise<WebhookError[]> {
    try {
      const { data: errors, error } = await supabaseService
        .from('webhook_logs')
        .select('webhook_id, event_type, error_message, created_at, retry_count')
        .eq('status', 'failed')
        .order('created_at', { ascending: false })
        .limit(limit);
  ...
  async checkHealth(): Promise<{
    ...
    try {
      ...
      if (metrics.averageProcessingTime >= 5000) {
        issues.push(`Slow processing time: ${metrics.averageProcessingTime.toFixed(2)}ms`);
      }

      return {
        healthy: checks.database && checks.webhookQueue && checks.errorRate && checks.processingTime,
        checks,
        issues,
      };
    } catch (error) {
  ...
  async getStatsForPeriod(
    ...
    try {
      ...
      const eventTypeCounts: Record<string, { success: number; total: number }> = {};
      ...
      const eventTypeBreakdown: Record<string, { count: number; successRate: number }> = {};
