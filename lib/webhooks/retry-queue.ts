...
export class WebhookRetryQueue {
	...
	static async add(
		...
		const { priority = "medium", maxAttempts = 5, delay = 0 } = options;
		...
		try {
			...
			console.log(
				`Added webhook ${webhookId} to retry queue (priority: ${priority})`,
			);
		} catch (error) {
	...
	private async processBatch(): Promise<void> {
		...
		try {
			...
			const processingPromises = jobs.map((job: any) => this.processJob(job));
	...
	private async processJob(job: WebhookRetryJob): Promise<void> {
		try {
			...
			if (result.success) {
				...
				console.error(
					`Failed to retry webhook ${job.webhook_id}:`,
					result.error,
				);

				// Check if we should requeue for later
				if (job.attempt_count < job.max_attempts) {
					const delay = 2 ** job.attempt_count * 1000; // Exponential backoff
			...
			if (job.attempt_count < job.max_attempts) {
				const delay = 2 ** job.attempt_count * 1000;
				await WebhookRetryQueue.add(
					job.webhook_id,
					job.event_type,
					job.payload,
	...
	static async getStats(): Promise<{
		...
		if (error || highError || mediumError || lowError || overdueError) {
			console.error(
				...
				error || highError || mediumError || lowError || overdueError,
			);
			return {
				pending: 0,
				highPriority: 0,
