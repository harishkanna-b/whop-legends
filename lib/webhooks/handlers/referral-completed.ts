...
import { updateUserStats, checkAndUnlockAchievements } from '@/lib/user-management';
...
export const handleReferralCompleted = async (
  ...
  try {
    ...
    const userUpdateResult = await updateUserStats(updatedReferral.referrer_id, {
    ...
    console.log(`Successfully processed referral.completed for referral ${referral.code}`);
    return {
      success: true,
      processed: true,
    };
