import { WhopWebhookEvent, WebhookProcessingResult } from '@/types/whop';
...
export const handleReferralCancelled = async (
  ...
  try {
    ...
    const { data: updatedReferral, error: referralError } = await supabaseService
      .from('referrals')
      .update({
        status: 'cancelled',
    ...
    console.log(`Successfully processed referral.cancelled for referral ${referral.code}`);
    return {
      success: true,
      processed: true,
    };
...
const adjustUserStatsForCancelledReferral = async (referral: any) => {
  try {
    ...
    const newTotalCommission = Math.max(0, user.total_commission - referral.commission_amount);
    ...
    console.log(`Adjusted stats for user ${referral.referrer_id}: referrals=${newTotalReferrals}, commission=${newTotalCommission}`);
  } catch (error) {
