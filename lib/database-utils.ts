import { type Database, supabase, supabaseService } from "./supabase-client";
...
export const createReferral = async (
	referralData: Database["public"]["Tables"]["referrals"]["Insert"],
) => {
	const { data, error } = await supabase()
		.from("referrals")
		.insert(referralData)
...
export const updateReferral = async (
	referralId: string,
	updates: Database["public"]["Tables"]["referrals"]["Update"],
) => {
	const { data, error } = await supabase()
		.from("referrals")
		.update(updates)
...
// Quest operations
export const getQuestsByCompany = async (
	companyId: string,
	isActive = true,
) => {
	const { data, error } = await supabase()
...
export const updateCreatorSettings = async (
	companyId: string,
	settings: Partial<Database["public"]["Tables"]["creator_settings"]["Update"]>,
) => {
	const { data, error } = await supabase()
		.from("creator_settings")
		.upsert({
...
export const subscribeToReferralUpdates = (
	userId: string,
	callback: (payload: Database["public"]["Tables"]["referrals"]["Row"]) => void,
) => {
	return supabase()
...
export const subscribeToQuestUpdates = (
	userId: string,
	callback: (payload: Database["public"]["Tables"]["user_quests"]["Row"]) => void,
) => {
	return supabase()
...
export const bulkCreateReferrals = async (
	referrals: Database["public"]["Tables"]["referrals"]["Insert"][],
) => {
	const { data, error } = await supabase()
		.from("referrals")
		.insert(referrals)
...
export const createLeaderboardSnapshot = async (snapshotData: {
...
export const getUserRankingHistory = async (
	userId: string,
...
export const createUserRankingEntry = async (entryData: {
	user_id: string;
	leaderboard_id: string;
	rank: number;
	score: number;
