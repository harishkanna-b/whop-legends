...
import { DatabaseUser } from '@/types/whop';
...
export const getXPProgress = (user: DatabaseUser): {
  currentXP: number;
  nextLevelXP: number;
  ...
  const progressPercentage = Math.round((progressXP / (nextLevelXP - currentLevelXP)) * 100);
