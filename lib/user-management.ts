...
import { DatabaseUser, UserUpdateData } from '@/types/whop';
...
): Promise<{ success: boolean; levelUp?: any; newStats?: any }> => {
  try {
    ...
    const newXP = (currentUser.experience_points || 0) + (updates.experience_points || 0);
    const newReferrals = (currentUser.total_referrals || 0) + (updates.total_referrals || 0);
    const newCommission = (currentUser.total_commission || 0) + (updates.total_commission || 0);
    ...
    // Check for level up
    const levelUpResult = await checkLevelUp(userId, updates.experience_points || 0);
...
export const checkAndUnlockAchievements = async (
  ...
  try {
    ...
    const { data: existingAchievements, error: achievementError } = await supabaseService
    ...
    if (user.total_referrals >= 50 && !existingIds.includes('fifty_referrals')) {
    ...
    if (user.total_commission >= 100 && !existingIds.includes('commission_100')) {
      await unlockAchievement(userId, 'commission_100');
      unlockedAchievements.push('$100 Commission');
    }

    if (user.total_commission >= 1000 && !existingIds.includes('commission_1000')) {
...
const unlockAchievement = async (userId: string, achievementId: string) => {
  try {
    ...
    const { data: achievement, error: achievementError } = await supabaseService
      .from('achievements')
...
export const getUserLeaderboardRank = async (userId: string) => {
  try {
    ...
    const { data: higherRankedUsers, error: rankError } = await supabaseService
      .from('users')
