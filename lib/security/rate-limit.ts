...
export function createRateLimitMiddleware(limiter: RateLimiter) {
  return async (req: any, res: any, next: () => void) => {
    try {
      ...
      res.setHeader('X-RateLimit-Remaining', result.limitInfo.remaining.toString());
      res.setHeader('X-RateLimit-Reset', result.limitInfo.resetTime.getTime().toString());

      if (!result.allowed) {
        res.status(429).json({
          error: 'Too Many Requests',
          message: 'Rate limit exceeded. Please try again later.',
          retryAfter: Math.ceil((result.limitInfo.resetTime.getTime() - Date.now()) / 1000)
        });
        return;
      }
