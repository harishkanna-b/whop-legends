...
export class WebhookSecurity {
  ...
  static async validateWebhook(
    ...
    try {
      ...
      if (this.config.allowedIps.length > 0) {
        securityChecks.ipAllowed = this.config.allowedIps.includes(clientIp);
      ...
      securityChecks.payloadSizeValid = rawBody.length <= this.config.maxPayloadSize;
      if (!securityChecks.payloadSizeValid) {
        riskScore += 0.2;
        error = `Payload size exceeds limit: ${rawBody.length} > ${this.config.maxPayloadSize}`;
      }

      // 4. Timestamp validation (if present in payload)
      const timestampValidation = this.validateTimestamp(rawBody);
      ...
      if (signature && process.env.WHOP_WEBHOOK_SECRET) {
        securityChecks.signatureValid = this.validateSignature(
          signature,
          rawBody,
      ...
      try {
        ...
        const payloadValidation = this.validatePayloadStructure(data);
      ...
      const securityScan = this.scanForSecurityThreats(data, rawBody);
      if (securityScan.threatDetected) {
        riskScore += 0.6;
        error = `Security threat detected: ${securityScan.threatType}`;
      }

      const isValid = riskScore < 0.3 && Object.values(securityChecks).every(check => check);
      ...
      this.logSecurityMetrics(clientIp, riskScore, securityChecks, Date.now() - startTime);
    }
  }
  ...
  private static checkRateLimit(ip: string): { allowed: boolean; remaining: number } {
    ...
    if (!record || now > record.resetTime) {
      // New window
      this.rateLimiter.set(ip, { count: 1, resetTime: now + windowSize });
    ...
    try {
      ...
      for (const sig of signatures) {
        if (sig.startsWith('sha256=')) {
          ...
          if (this.constantTimeEqual(calculatedSignature, expectedSignature)) {
            return true;
          }
        }
      }
   ...
   */
  private static validateTimestamp(payload: string): { valid: boolean; error?: string } {
    try {
   ...
   */
  private static validatePayloadStructure(data: any): { valid: boolean; error?: string } {
    ...
    if (data.action === 'payment.succeeded') {
      ...
      // Validate data types
      if (typeof data.data.final_amount !== 'number' || data.data.final_amount <= 0) {
        return { valid: false, error: 'Invalid payment amount' };
      }

      if (typeof data.data.currency !== 'string' || data.data.currency.length !== 3) {
   ...
   */
  private static scanForSecurityThreats(data: any, rawBody: string): { threatDetected: boolean; threatType?: string } {
    ...
    const depthCheck = this.checkObjectDepth(data);
   ...
   */
  private static checkObjectDepth(obj: any, depth: number = 0, maxDepth: number = 10): { tooDeep: boolean } {
    if (depth > maxDepth) {
      return { tooDeep: true };
    }
    ...
    for (const value of Object.values(obj)) {
      if (typeof value === 'object' && value !== null) {
        const result = this.checkObjectDepth(value, depth + 1, maxDepth);
        if (result.tooDeep) {
          return result;
        }
      }
  ...
  static getSecurityStats(): {
    ...
    const totalRequests = Array.from(this.rateLimiter.values())
      .reduce((sum, record) => sum + record.count, 0);

    const highRiskRequests = Array.from(this.rateLimiter.values())
      .filter(record => record.count > 50).length;

    const blockedIps = Array.from(this.rateLimiter.entries())
    ...
    return {
      ...
      blockedIps,
      rateLimitStats: {
        activeIps: this.rateLimiter.size,
