...
export class SecurityValidator {
	...
	static validateUserId(userId: string): ValidationResult {
		...
		return {
			isValid: true,
			sanitized: SecurityValidator.sanitizeString(userId),
		};
	}
	...
	static validateCompanyId(companyId: string): ValidationResult {
		...
		return {
			isValid: true,
			sanitized: SecurityValidator.sanitizeString(companyId),
		};
	}
	...
	static validateQuestParams(params: {
		...
		if (params.title) {
			if (typeof params.title !== "string") {
				...
				sanitized.title = SecurityValidator.sanitizeString(params.title);
			}
		}
		...
		if (params.description) {
			if (typeof params.description !== "string") {
				...
				sanitized.description = SecurityValidator.sanitizeString(
					params.description,
				);
			}
		}
		...
		if (params.target_value !== undefined) {
			if (
				typeof params.target_value !== "number" ||
				params.target_value < 1 ||
				params.target_value > 10000
			) {
		...
		if (params.reward_xp !== undefined) {
			if (
				typeof params.reward_xp !== "number" ||
				params.reward_xp < 0 ||
				params.reward_xp > 10000
			) {
		...
		if (params.reward_commission !== undefined) {
			if (
				typeof params.reward_commission !== "number" ||
				params.reward_commission < 0 ||
				params.reward_commission > 1000
			) {
				errors.push("Reward commission must be a number between 0 and 1000");
			} else {
				sanitized.reward_commission =
					Math.round(params.reward_commission * 100) / 100; // Round to 2 decimal places
			}
		}
		...
		if (params.quest_type) {
			...
			if (!validQuestTypes.includes(params.quest_type)) {
				errors.push(
					`Invalid quest type. Must be one of: ${validQuestTypes.join(", ")}`,
				);
			} else {
	...
	static validateAnalyticsParams(params: {
		...
		if (params.metrics) {
			if (!Array.isArray(params.metrics)) {
				...
				const invalidMetrics = params.metrics.filter(
		...
		if (params.group_by) {
			...
			if (!validGroupBy.includes(params.group_by)) {
				errors.push(
					`Invalid group_by. Must be one of: ${validGroupBy.join(", ")}`,
				);
			} else {
