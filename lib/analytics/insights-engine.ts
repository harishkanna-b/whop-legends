...
export class InsightsEngine {
  ...
  private initializeDefaultConfigs() {
    ...
    this.addConfig({
      ...
      actions: [
        { type: 'notification', target: 'creator', template: 'Conversion rate is below 20%. Consider reviewing referral strategies.' }
      ],
      priority: 'high',
      enabled: true
    });
    ...
    this.addConfig({
      ...
      actions: [
        { type: 'notification', target: 'both', template: 'Exceptional performance detected!' },
    ...
    this.addConfig({
      ...
      actions: [
        { type: 'notification', target: 'creator', template: 'Member engagement is declining. Consider engagement campaigns.' },
    ...
    this.addConfig({
      ...
      conditions: [
        ...
        { field: 'quest_completion_rate', operator: 'gt', value: 70, weight: 0.3 },
    ...
    this.addConfig({
      ...
      category: 'retention',
      conditions: [
        { field: 'days_since_last_activity', operator: 'gt', value: 30, weight: 0.4 },
        { field: 'engagement_score', operator: 'lt', value: 30, weight: 0.3 },
        { field: 'retention_rate', operator: 'lt', value: 50, weight: 0.3 }
      ],
      actions: [
        { type: 'notification', target: 'creator', template: 'Members at risk of churn detected.' },
        { type: 'message', target: 'member', template: 'reactivation_campaign' }
      ],
      priority: 'critical',
      enabled: true
    });
    ...
    this.addConfig({
      ...
      actions: [
        { type: 'notification', target: 'creator', template: 'Exceptional growth rate detected!' }
      ],
      priority: 'medium',
      enabled: true
    });
    ...
    this.addConfig({
      ...
      category: 'revenue',
      conditions: [
        { field: 'revenue_change_percent', operator: 'lt', value: -20, weight: 0.5 },
        { field: 'revenue_volatility', operator: 'gt', value: 30, weight: 0.5 }
      ],
      actions: [
        { type: 'notification', target: 'creator', template: 'Unusual revenue pattern detected. Review recent changes.' }
      ],
      priority: 'high',
      enabled: true
    });
  ...
  async generateInsights(companyId: string, timeframe: '7d' | '30d' | '90d' = '30d'): Promise<Insight[]> {
    const cacheKey = `insights_${companyId}_${timeframe}`;
    const cached = this.cache.get(cacheKey);

    if (cached && cached[0]) {
      const createdAt = cached[0].created_at;
      const cacheAge = createdAt ? Date.now() - new Date(createdAt).getTime() : 0;
    ...
    for (const [configId, config] of this.configs.entries()) {
      ...
      const matchingInsights = await this.evaluateConfig(config, companyData, companyId);
  ...
  private async getCompanyData(companyId: string, timeframe: string) {
    ...
    const totalCommission = members?.reduce((sum, m) => sum + (m.total_commission || 0), 0) || 0;
    const totalReferrals = members?.reduce((sum, m) => sum + (m.total_referrals || 0), 0) || 0;
    const avgConversionRate = members?.reduce((sum, m) => sum + (m.conversion_rate || 0), 0) / (members?.length || 1) || 0;
    const avgEngagement = members?.reduce((sum, m) => sum + (m.engagement_score || 0), 0) / (members?.length || 1) || 0;

    // Calculate trends
    const dailyData = analytics || [];
    const revenueTrend = this.calculateTrend(dailyData.map(d => d.total_commission || 0));
    const memberTrend = this.calculateTrend(dailyData.map(d => d.active_users || 0));
  ...
  private async evaluateConfig(config: InsightConfig, companyData: any, companyId: string): Promise<Insight[]> {
    const insights: Insight[] = [];
    ...
    for (const member of companyData.members) {
      ...
      if (passes) {
        const insight = await this.createInsight(config, context, companyId, member.user_id);
    ...
    const companyPasses = await this.evaluateConditions(
      config.conditions.filter(c => !c.field.startsWith('member_')),
      companyContext
    );

    if (companyPasses) {
      const insight = await this.createInsight(config, companyContext, companyId);
      insights.push(insight);
    }
  ...
  private async evaluateConditions(conditions: InsightCondition[], context: any): Promise<boolean> {
  ...
  private async evaluateCondition(condition: InsightCondition, context: any): Promise<boolean> {
  ...
  private async createInsight(config: InsightConfig, context: any, companyId: string, userId?: string): Promise<Insight> {
  ...
  private generateInsightDescription(config: InsightConfig, context: any): string {
    const templates = {
  ...
  async getInsights(companyId: string, filters?: {
    type?: string;
    category?: string;
    priority?: string;
    acknowledged?: boolean;
  ...
  async getInsightStats(companyId: string): Promise<{
    ...
    return {
      ...
      acknowledged_insights: allInsights.filter(i => i.acknowledged).length,
  ...
  private async executeAction(action: InsightAction, insight: Insight): Promise<void> {
    switch (action.type) {
      case 'notification':
