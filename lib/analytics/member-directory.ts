...
export interface MemberFilter {
  ...
  sort_by?: 'total_commission' | 'total_referrals' | 'level' | 'engagement_score' | 'last_active';
  sort_order?: 'asc' | 'desc';
}
...
export class MemberDirectory {
  ...
  async searchMembers(companyId: string, filters: MemberFilter, page: number = 1): Promise<{
    ...
    if (filters.level_range) {
      query = query.gte('level', filters.level_range[0]).lte('level', filters.level_range[1]);
    }

    if (filters.commission_range) {
      query = query.gte('total_commission', filters.commission_range[0]).lte('total_commission', filters.commission_range[1]);
    }

    if (filters.referral_range) {
      query = query.gte('total_referrals', filters.referral_range[0]).lte('total_referrals', filters.referral_range[1]);
    }
    ...
    if (filters.join_date_range) {
      query = query.gte('join_date', filters.join_date_range[0]).lte('join_date', filters.join_date_range[1]);
    }
  ...
  async getMemberDetails(memberId: string, companyId: string): Promise<MemberSearchResult | null> {
  ...
  async getMemberAnalytics(memberId: string, companyId: string, timeframe: '7d' | '30d' | '90d' = '30d') {
    ...
    return {
      ...
      summary: {
        ...
        converted_referrals: referrals?.filter(r => r.converted_at).length || 0,
        total_commission: referrals?.reduce((sum, r) => sum + (r.commission_amount || 0), 0) || 0,
        completed_quests: quests?.filter(q => q.is_completed).length || 0,
        claimed_rewards: quests?.filter(q => q.reward_claimed).length || 0
      }
    };
  }

  async exportMembers(companyId: string, options: MemberExportOptions): Promise<string> {
  ...
  async getMemberRank(memberId: string, companyId: string): Promise<number> {
    const { data: higherPerformers, error } = await supabase
      ...
      .eq('company_id', companyId)
      .gt('total_commission',
        supabase.from('member_performance_stats')
  ...
  async getMemberBadges(memberId: string, companyId: string): Promise<string[]> {
  ...
  async getMemberStats(companyId: string): Promise<{
    ...
    const averageLevel = members?.reduce((sum, m) => sum + (m.level || 0), 0) / (members?.length || 1);
    ...
    const topCharacterClass = Object.entries(classCounts).reduce((max, [cls, count]) =>
      (count as number) > max.count ? { class: cls, count: count as number } : max, { class: '', count: 0 }
    ).class;
    ...
    const newMembersThisMonth = members?.filter(m =>
      new Date(m.join_date) >= startOfMonth
    ).length || 0;
  ...
  async bulkUpdateMembers(companyId: string, updates: Array<{
    user_id: string;
    updates: Partial<MemberSearchResult>;
  }>): Promise<void> {
    const updatePromises = updates.map(async ({ user_id, updates }) => {
  ...
  async getMemberActivityTimeline(memberId: string, companyId: string, limit: number = 50) {
  ...
  private exportToJSON(members: MemberSearchResult[], fields: string[]): string {
