...
export class AnalyticsEngine {
	...
	private async getAnalyticsMetrics(
		...
		const referralValues = dailyData.map((d: any) => d.total_referrals || 0);
		...
		const commissionValues = dailyData.map((d: any) => d.total_commission || 0);
		...
		const engagementValues = dailyData.map((d: any) => d.active_users || 0);
		...
		const conversionValues = dailyData.map((d: any) => d.conversion_rate || 0);
	...
	private async generateCreatorInsights(
		...
		const avgEngagement =
			performance.reduce(
				(sum: any, p: any) => sum + (p.engagement_score || 0),
				0,
			) / performance.length;
		if (avgEngagement < 50) {
			insights.push({
		...
		const topPerformers = performance.filter(
			(p: any) => (p.total_commission || 0) > 1000,
		);
		...
		const avgConversion =
			performance.reduce(
				(sum: any, p: any) => sum + (p.conversion_rate || 0),
				0,
			) / performance.length;
		if (avgConversion > 30) {
			insights.push({
	...
	async getMemberDetails(memberId: string, companyId: string) {
		...
		return {
			...
			quest_stats: {
				...
				completed_quests:
					questStats?.filter((q: any) => q.is_completed).length || 0,
				claimed_rewards:
					questStats?.filter((q: any) => q.reward_claimed).length || 0,
			},
		};
	}
	...
	async generateReport(
		...
		switch (reportType) {
			case "performance":
				...
				break;
			case "engagement":
				report.data = await this.generateEngagementReport(
					companyId,
					startDate,
					endDate,
				);
				break;
			case "retention":
				report.data = await this.generateRetentionReport(
					companyId,
					startDate,
					endDate,
				);
				break;
			case "growth":
				report.data = await this.generateGrowthReport(
					companyId,
					startDate,
					endDate,
				);
	...
	private async generatePerformanceReport(
		...
		return {
			...
			average_commission:
				performance?.reduce(
					(sum: any, p: any) => sum + (p.total_commission || 0),
					0,
				) / (performance?.length || 1),
			top_performer: performance?.reduce(
				(max: any, p: any) =>
					(p.total_commission || 0) > (max.total_commission || 0) ? p : max,
				performance[0],
			),
		...
		return {
			retention_rate:
				members && members.length > 0
					? members.reduce(
							(sum: any, m: any) =>
								sum + ("retention_rate" in m ? m.retention_rate || 0 : 0),
							0,
						) / members.length
					: 0,
			churn_rate:
				members && members.length > 0
					? (members.filter(
							(m: any) => "retention_rate" in m && (m.retention_rate || 0) < 50,
						).length /
							members.length) *
						100
					: 0,
			average_lifespan:
				members && members.length > 0
					? members.reduce((sum: any, m: any) => {
							const lifespan =
								"join_date" in m
									? new Date().getTime() - new Date(m.join_date).getTime()
									: 0;
							return sum + lifespan;
						}, 0) /
						members.length /
