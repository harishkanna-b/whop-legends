...
export class Logger {
  ...
  component(component: string) {
    return {
      debug: (message: string, operation: string, metadata: Record<string, any> = {}) => {
        this.log('debug', message, { ...metadata, component, operation });
      },
      info: (message: string, operation: string, metadata: Record<string, any> = {}) => {
        this.log('info', message, { ...metadata, component, operation });
      },
      warn: (message: string, operation: string, metadata: Record<string, any> = {}) => {
        this.log('warn', message, { ...metadata, component, operation });
      },
      error: (message: string, operation: string, error?: any, metadata: Record<string, any> = {}) => {
        this.log('error', message, { ...metadata, component, operation, error });
      },
      critical: (message: string, operation: string, error?: any, metadata: Record<string, any> = {}) => {
        this.log('critical', message, { ...metadata, component, operation, error });
      }
    };
  }
  ...
  health(component: string, status: 'healthy' | 'degraded' | 'unhealthy', metrics: Record<string, any> = {}): void {
  ...
  private logToConsole(entry: LogEntry): void {
    const { timestamp, level, message, component, operation, requestId, duration, error } = entry;
