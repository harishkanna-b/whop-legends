...
export const whopApi = {
	...
	async getPayment(paymentId: string) {
		try {
			...
			throw new Error(
				`Failed to fetch payment ${paymentId}: ${(error as Error).message}`,
			);
		}
	},
	...
	async getUserLedger(userId: string) {
		try {
			...
			throw new Error(
				`Failed to fetch user ledger ${userId}: ${(error as Error).message}`,
			);
		}
	},
	...
	async getUserReceipts(
		...
		try {
			...
			throw new Error(
				`Failed to fetch user receipts ${userId}: ${(error as Error).message}`,
			);
		}
	},
};
