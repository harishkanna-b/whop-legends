...
export class RankingEngine {
	...
	async calculateLeaderboard(
		...
		const scoredMembers = filteredMembers.map((member: any) => {
	...
	async saveRankingHistory(
		...
		const historyRecords = entries.map((entry: any) => ({
	...
	async getLeaderboardStatistics(leaderboardId: string): Promise<{
		...
		const score_distribution = scoreRanges.map((range: any) => ({
			range: range.label,
			count: entries.filter(
				(e: any) => e.score >= range.min && e.score < range.max,
			).length,
			percentage:
				(entries.filter((e: any) => e.score >= range.min && e.score < range.max)
					.length /
					entries.length) *
				100,
		}));
		...
		const recent_activity = entries.slice(0, 10).map((entry: any) => ({
			user_id: entry.user_id,
	...
	private async getLeaderboardConfig(
		...
		const configs = await this.getLeaderboardConfigs(companyId);
		return configs.find((c: any) => c.id === leaderboardId) || null;
	}
}
