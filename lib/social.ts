...
export interface SocialActivity {
  id: string
  userId: string
  type: 'friend_request' | 'friend_accept' | 'team_join' | 'team_leave' | 'achievement' | 'milestone'
  description: string
  metadata?: any
  createdAt: string
  isPublic: boolean
...
export class SocialManager {
  ...
  // Friend Management
  async sendFriendRequest(requesterId: string, recipientId: string, message?: string): Promise<FriendRequest> {
    try {
      // Check if request already exists
      const { data: existingRequest } = await supabase
        .from('friend_requests')
        .select('*')
        .or(`(requester_id.eq.${requesterId} AND recipient_id.eq.${recipientId})`)
        .or(`(requester_id.eq.${recipientId} AND recipient_id.eq.${requesterId})`)
  ...
  async respondToFriendRequest(requestId: string, userId: string, accept: boolean): Promise<void> {
    try {
      ...
      if (accept) {
        ...
        // Log social activity
        await this.logSocialActivity(request.requester_id, 'friend_accept', `You are now friends with ${request.recipient_id}`)
        await this.logSocialActivity(request.recipient_id, 'friend_accept', `You are now friends with ${request.requester_id}`)
      }
    } catch (error) {
  ...
  async getFriends(userId: string, status: 'accepted' | 'pending' = 'accepted'): Promise<Friend[]> {
    try {
      const { data, error } = await supabase
        .from('friends')
        .select(`
      ...
      return data.map(item => this.mapFriend(item))
    } catch (error) {
  ...
  async getFriendRequests(userId: string, type: 'sent' | 'received' = 'received'): Promise<FriendRequest[]> {
    try {
      ...
      return data.map(item => this.mapFriendRequest(item))
    } catch (error) {
  ...
  async removeFriend(userId: string, friendId: string): Promise<void> {
    try {
      ...
      // Log social activity
      await this.logSocialActivity(userId, 'friend_remove', `You are no longer friends with ${friendId}`)
    } catch (error) {
  ...
  async createTeam(
    ...
    try {
      ...
      await supabase.from('team_members').insert({
        ...
        user_id: leaderId,
        role: 'leader',
        contributions: { referrals: 0, value: 0, invites: 0, achievements: 0 },
        is_active: true
      })

      // Log social activity
      await this.logSocialActivity(leaderId, 'team_create', `Created team: ${name}`)
  ...
  async joinTeam(userId: string, teamId: string, inviteCode?: string): Promise<void> {
    try {
      ...
      // Log social activity
      await this.logSocialActivity(userId, 'team_join', `Joined team: ${team.name}`)
    } catch (error) {
  ...
  async getTeams(userId: string, type: 'member' | 'all' = 'member'): Promise<Team[]> {
    try {
      ...
      return data.map(team => this.mapTeam(team))
    } catch (error) {
  ...
  async getTeamMembers(teamId: string): Promise<TeamMember[]> {
    try {
      ...
      return data.map(member => this.mapTeamMember(member))
    } catch (error) {
  ...
  async inviteToTeam(inviterId: string, inviteeId: string, teamId: string, message?: string): Promise<void> {
    try {
      const { data: team, error: teamError } = await supabase
        .from('teams')
        .select('*')
      ...
      // Log social activity
      await this.logSocialActivity(inviterId, 'team_invite', `Invited ${inviteeId} to join ${team.name}`)
    } catch (error) {
  ...
  async getSocialActivity(userId: string, limit: number = 20): Promise<SocialActivity[]> {
    try {
      ...
      return data.map(activity => this.mapSocialActivity(activity))
    } catch (error) {
  ...
  private async logSocialActivity(userId: string, type: string, description: string, metadata?: any): Promise<void> {
    try {
