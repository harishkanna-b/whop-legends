...
export class ReferralManager {
  ...
  async getUserReferrals(userId: string, timeframe: '7d' | '30d' | '90d' | '1y' | 'all' = 'all'): Promise<Referral[]> {
    try {
      let query = supabase
        .from('referrals')
        .select(`
      ...
      return referrals.map(referral => this.mapReferral({
        ...referral,
  ...
  async getReferralStats(userId: string, timeframe: '7d' | '30d' | '90d' | '1y' | 'all' = 'all'): Promise<ReferralStats> {
    try {
      ...
      const stats: ReferralStats = {
        ...
        averageCommission: referrals.length > 0 ? referrals.reduce((sum, r) => sum + r.commission, 0) / referrals.length : 0,
        conversionRate: referrals.length > 0 ? referrals.filter(r => r.status === 'completed').length / referrals.length : 0,
        averageValue: referrals.length > 0 ? referrals.reduce((sum, r) => sum + r.value, 0) / referrals.length : 0,
        topSources: [],
        monthlyTrend: [],
        performanceMetrics: {
      ...
      const sourceMap = new Map<string, { count: number; value: number; completed: number }>()
      referrals.forEach(referral => {
        const source = referral.source || 'direct'
        const current = sourceMap.get(source) || { count: 0, value: 0, completed: 0 }
        current.count++
      ...
      const monthlyMap = new Map<string, { referrals: number; completed: number; value: number; commission: number }>()
      referrals.forEach(referral => {
        const month = new Date(referral.createdAt).toISOString().substring(0, 7) // YYYY-MM
        const current = monthlyMap.get(month) || { referrals: 0, completed: 0, value: 0, commission: 0 }
        current.referrals++
        current.value += referral.value
      ...
      if (stats.monthlyTrend.length > 0) {
        const sortedByValue = [...stats.monthlyTrend].sort((a, b) => b.value - a.value)
        stats.performanceMetrics.bestMonth = sortedByValue[0]
        stats.performanceMetrics.worstMonth = sortedByValue[sortedByValue.length - 1]
        stats.performanceMetrics.averageMonthlyReferrals = stats.totalReferrals / stats.monthlyTrend.length
        stats.performanceMetrics.averageMonthlyValue = stats.totalValue / stats.monthlyTrend.length
        ...
        if (stats.monthlyTrend.length >= 2) {
          ...
          stats.performanceMetrics.growthRate = previous.value > 0 ?
            ((recent.value - previous.value) / previous.value) * 100 : 0
        }
      }
  ...
  async trackReferralClick(code: string, clickData: {
    ip?: string
    userAgent?: string
    referer?: string
    utmSource?: string
    ...
    try {
      ...
      const { data: existingClick, error: clickError } = await supabase
  ...
  async getReferralAnalytics(userId: string, timeframe: '7d' | '30d' | '90d' | '1y' = '30d'): Promise<ReferralAnalytics> {
    try {
      const days = parseInt(timeframe.replace('d', ''))
      const cutoffDate = new Date(Date.now() - days * 24 * 60 * 60 * 1000)
      ...
      const analytics: ReferralAnalytics = {
        ...
        uniqueClicks: clickData.filter(c => c.is_unique).length,
        conversions: conversions.length,
        conversionRate: clickData.length > 0 ? conversions.length / clickData.length : 0,
      ...
      const browserMap = new Map<string, { clicks: number; conversions: number }>()
      clickData.forEach(click => {
        const userAgent = click.user_agent || ''
        let browser = 'Unknown'
        ...
        const current = browserMap.get(browser) || { clicks: 0, conversions: 0 }
        current.clicks++
  ...
  private async generateUniqueCode(userId: string, context?: string): Promise<string> {
