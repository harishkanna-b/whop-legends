...
import type { QuestReward } from "@/lib/types/quest-types";
...
export class RewardManager {
	...
	static async distributeQuestRewards(
		...
		try {
			...
			if (questError || !quest) {
				console.error(
					"Error fetching quest for reward distribution:",
					questError,
				);
				return false;
			}
			...
			if (userError || !user) {
				console.error(
					"Error fetching user for reward distribution:",
					userError,
				);
				return false;
			}
			...
			const classMultiplier = RewardManager.getCharacterClassMultiplier(
				user.character_class,
			);
			const finalXP = Math.round(quest.reward_xp * classMultiplier.xp);
			const finalCommission =
				quest.reward_commission * classMultiplier.commission;
			...
			await RewardManager.logRewardDistribution(userId, questId, {
				xp: finalXP,
			...
			await RewardManager.checkAchievementUnlock(
				userId,
				user,
				finalXP,
				finalCommission,
	...
	static async calculateQuestRewards(
		...
		return {
			xp: finalXP,
			commission: finalCommission,
			achievements: await RewardManager.getPotentialAchievements(userId, quest),
		};
	}
	...
	static async bulkDistributeRewards(userId: string): Promise<{
		...
		const { data: completedQuests, error: questError } = await supabaseService()
			.from("user_quests")
			.select("*, quest:quests(*)")
			.eq("user_id", userId)
		...
		for (const userQuest of completedQuests) {
			const success = await RewardManager.distributeQuestRewards(
				userId,
				userQuest.quest_id,
			);
			...
			if (success) {
				...
				result.errors.push(
					`Failed to distribute rewards for quest ${userQuest.quest_id}`,
				);
			}
		}
	...
	> {
      ...
      `)
		...
		return data.map((item: Database["public"]["Tables"]["user_quests"]["Row"]) => ({
			id: item.id,
	...
	static async getRewardStats(userId: string): Promise<{
		...
		const { data: completedQuests, error } = await supabaseService()
			.from("user_quests")
			.select(`
        reward_claimed_at,
      ...
      `)
		...
		const totalXP = completedQuests.reduce(
			(sum: number, q: Database["public"]["Tables"]["user_quests"]["Row"]) => sum + (q.quest?.[0]?.reward_xp || 0),
			0,
		);
		const totalCommission = completedQuests.reduce(
			(sum: number, q: Database["public"]["Tables"]["user_quests"]["Row"]) => sum + (q.quest?.[0]?.reward_commission || 0),
			0,
		);
		const totalQuests = completedQuests.length;

		const typeStats: Record<string, { total: number; count: number }> = {};
		completedQuests.forEach((q: Database["public"]["Tables"]["user_quests"]["Row"]) => {
		...
		const mostProfitableType =
			Object.entries(typeStats).sort(
				([, a], [, b]) => b.total - a.total,
			)[0]?.[0] || "none";
		...
		return {
			...
			average_commission_per_quest:
				totalQuests > 0 ? totalCommission / totalQuests : 0,
			most_profitable_quest_type: mostProfitableType,
			reward_streak: RewardManager.calculateRewardStreak(completedQuests),
		};
	}

	// Private helper methods
	private static getCharacterClassMultiplier(characterClass: string): {
		xp: number;
		commission: number;
	} {
	...
	private static async logRewardDistribution(
		...
		console.log(
			`Reward distributed: User ${userId}, Quest ${questId}, XP: ${rewards.xp}, Commission: ${rewards.commission}`,
		);
	}
	...
	private static async getPotentialAchievements(
		userId: string,
	...
	private static calculateRewardStreak(completedQuests: Database["public"]["Tables"]["user_quests"]["Row"][]): number {
		if (completedQuests.length === 0) return 0;

		const sortedQuests = completedQuests.sort(
			(a, b) =>
				new Date(b.reward_claimed_at).getTime() -
				new Date(a.reward_claimed_at).getTime(),
		);
	 ...
	 */
	static async createMilestoneReward(
		userId: string,
		...
		try {
			...
			console.log(
				`Creating milestone reward for user ${userId}: ${milestoneType} ${milestoneValue}`,
			);
			return true;
		} catch (error) {
	 ...
	 */
	static async getPendingRewards(userId: string): Promise<
		Array<{
