...
import { Quest, QuestTemplate, QuestGenerationConfig } from '@/lib/types/quest-types';
...
export class QuestGenerator {
  ...
  static async generateDailyQuests(companyId: string): Promise<void> {
    ...
    if (userError || !users) {
      console.error('Error fetching users for daily quest generation:', userError);
      return;
    }
    ...
    for (const user of users) {
      ...
      const quests = await this.generateQuestsForUser(config);
      await this.assignQuestsToUser(user.id, quests);
    }
  }
  ...
  static async generateWeeklyQuests(companyId: string): Promise<void> {
    ...
    if (userError || !users) {
      console.error('Error fetching users for weekly quest generation:', userError);
      return;
    }
  ...
  static async generateMonthlyQuests(companyId: string): Promise<void> {
    ...
    if (userError || !users) {
      console.error('Error fetching users for monthly quest generation:', userError);
      return;
    }
   ...
   */
  static async generateQuestsForUser(config: QuestGenerationConfig): Promise<Quest[]> {
    const quests: Quest[] = [];

    // Get company-specific quest templates
    const { data: companySettings, error: settingsError } = await supabaseService
    ...
    const selectedTemplates = this.selectTemplatesForUser(templates, config);
  ...
  static async createQuestTemplates(companyId: string): Promise<void> {
    const defaultTemplates = this.getDefaultQuestTemplates();
  ...
  private static async assignQuestsToUser(userId: string, quests: Quest[]): Promise<void> {
  ...
  private static selectTemplatesForUser(templates: QuestTemplate[], config: QuestGenerationConfig): QuestTemplate[] {
    const questCount = this.getQuestCount(config.quest_type);
    const suitableTemplates = templates.filter(template => {
      // Filter templates based on user level and difficulty appropriateness
      const difficultyLevel = this.getDifficultyLevel(template.difficulty);
  ...
  private static async createQuestFromTemplate(template: QuestTemplate, config: QuestGenerationConfig): Promise<Quest | null> {
    ...
    const rewardMultiplier = this.getRewardMultiplier(config.character_class);
    const rewardXP = Math.round(baseReward * rewardMultiplier * (1 + config.user_level * 0.1));

    const quest: Quest = {
      id: `quest_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      company_id: config.company_id,
      title: this.generateQuestTitle(template, config),
      description: this.generateQuestDescription(template, targetValue),
  ...
  private static generateDefaultQuests(config: QuestGenerationConfig): Quest[] {
    const defaultQuestDefs = this.getDefaultQuestDefinitions(config.quest_type);
    const quests: Quest[] = [];

    for (let i = 0; i < Math.min(defaultQuestDefs.length, this.getQuestCount(config.quest_type)); i++) {
  ...
  private static getDefaultQuestDefinitions(questType: 'daily' | 'weekly' | 'monthly'): any[] {
  ...
  private static generateQuestTitle(template: QuestTemplate, config: QuestGenerationConfig): string {
    const titles = [
  ...
  private static generateQuestDescription(template: QuestTemplate, targetValue: number): string {
    const descriptions = [
  ...
  public static async generateQuestsFromTemplates(config: any, templates: QuestTemplate[]): Promise<Quest[]> {
  ...
  public static generateDynamicQuests(userStats: any, companyId: string): Quest[] {
    const quests: Quest[] = [];
    ...
    for (let i = 0; i < questCount; i++) {
      const difficulty = this.getDifficultyFromLevel(userStats.level);
      const quest = this.createBalancedQuest(difficulty, companyId);
      quests.push(quest);
    }
  ...
  public static createBalancedQuest(difficulty: string, companyId: string, questType: 'daily' | 'weekly' | 'monthly' = 'daily'): Quest {
    const baseRewards = {
      easy: { xp: 50, commission: 2.50 },
      medium: { xp: 150, commission: 7.50 },
      hard: { xp: 300, commission: 15.00 },
      epic: { xp: 500, commission: 25.00 }
    };
    ...
    const rewards = baseRewards[difficulty as keyof typeof baseRewards] || baseRewards.easy;
    const targets = baseTargets[difficulty as keyof typeof baseTargets] || baseTargets.easy;
    ...
    return {
      ...
      target_value: Math.random() > 0.5 ? targets.referrals : targets.commission,
      reward_xp: rewards.xp,
      reward_commission: rewards.commission,
      is_active: true,
      start_date: new Date().toISOString(),
      end_date: this.getQuestEndDate(questType),
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    };
  }

  public static scaleQuestToLevel(requirements: { base_target_value: number; scaling_factor: number }, level: number): number {
    const levelBonus = (level - 1) * requirements.scaling_factor;
    return Math.round(requirements.base_target_value * (1 + levelBonus));
  }

  public static applyCharacterClassMultiplier(baseReward: { xp: number; commission: number }, characterClass: string): { xp: number; commission: number } {
    const multiplier = this.getRewardMultiplier(characterClass);
    return {
