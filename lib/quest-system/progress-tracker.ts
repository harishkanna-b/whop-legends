...
import { QuestProgress, UserQuest, QuestStatus } from '@/lib/types/quest-types';
...
export class ProgressTracker {
   ...
   */
  static async trackUserProgress(userId: string, callback: (progress: QuestProgress[]) => void): Promise<RealtimeChannel> {
    ...
    const initialProgress = await this.getUserQuestProgress(userId);
  ...
  static async getUserQuestProgress(userId: string): Promise<QuestProgress[]> {
    ...
    return userQuests.map(userQuest => {
      const current_value = this.calculateCurrentProgress(userQuest.quest!, userStats);
      ...
      return {
        ...
        time_remaining: time_remaining ? parseInt(time_remaining) * 60 : undefined // convert minutes to seconds
      };
    });
  }
   ...
   */
  static async bulkUpdateProgress(userId: string, questIds: string[]): Promise<void> {
    const userStats = await this.getUserStats(userId);
    ...
    for (const questId of questIds) {
      ...
      const currentProgress = this.calculateCurrentProgress(userQuest.quest!, userStats);
  ...
  static async checkQuestCompletions(userId: string): Promise<UserQuest[]> {
    ...
    for (const userQuest of completedQuests) {
      if (userQuest.quest!.reward_xp > 0 || userQuest.quest!.reward_commission > 0) {
  ...
  static async getProgressSummary(userId: string): Promise<{
    ...
    const inProgressQuests = userQuests.filter(q => !q.is_completed);

    // Calculate rewards from completed quests (would need to join with quests table)
    const total_xp_earned = completedQuests.reduce((sum, q) => sum + 100, 0); // Placeholder
    const total_commission_earned = completedQuests.reduce((sum, q) => sum + 5, 0); // Placeholder

    return {
      total_quests: userQuests.length,
      completed_quests: completedQuests.length,
      in_progress_quests: inProgressQuests.length,
      completion_rate: userQuests.length > 0 ? (completedQuests.length / userQuests.length) * 100 : 0,
      total_xp_earned,
   ...
   */
  static async getProgressHistory(userId: string, days: number = 30): Promise<{
    date: string;
    completed_quests: number;
    ...
    // Group by date
    const history: Record<string, { completed_quests: number; total_progress: number }> = {};
  ...
  static async expireOverdueQuests(): Promise<void> {
    ...
    const { data: overdueQuests, error } = await supabase
      .rpc('expire_overdue_quests', { current_time: now });

    if (error) {
      console.error('Error expiring overdue quests:', error);
      // Fallback to manual processing if RPC doesn't exist
      await this.expireOverdueQuestsFallback(now);
    } else {
  ...
  private static async expireOverdueQuestsFallback(now: string): Promise<void> {
    try {
      ...
      const overdueQuests = userQuests.filter(uq =>
        uq.quest && new Date(uq.quest.end_date) < new Date(now)
      );
      ...
      console.log(`Expired ${overdueQuests.length} overdue quests (fallback method)`);
    } catch (error) {
