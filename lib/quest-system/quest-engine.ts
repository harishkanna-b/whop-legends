...
export class QuestEngine {
   ...
   */
  static async generateUserQuests(config: QuestGenerationConfig): Promise<Quest[]> {
    return this.generateQuestsForType(config);
  }

  /**
   * Generate daily quests with appropriate difficulty scaling
   */
  static async generateDailyQuests(config: QuestGenerationConfig): Promise<Quest[]> {
    const dailyConfig = { ...config, quest_type: 'daily' as 'daily' | 'weekly' | 'monthly' };
    return this.generateQuestsForType(dailyConfig);
  }

  /**
   * Generate weekly quests with higher rewards and complexity
   */
  static async generateWeeklyQuests(config: QuestGenerationConfig): Promise<Quest[]> {
    const weeklyConfig = {
      ...config,
      quest_type: 'weekly' as 'daily' | 'weekly' | 'monthly',
      difficulty_preferences: ['medium', 'hard'] as QuestDifficulty[]
    };
    return this.generateQuestsForType(weeklyConfig);
  }

  /**
   * Generate monthly quests with significant challenges and prestige rewards
   */
  static async generateMonthlyQuests(config: QuestGenerationConfig): Promise<Quest[]> {
    const monthlyConfig = {
      ...config,
      quest_type: 'monthly' as 'daily' | 'weekly' | 'monthly',
      difficulty_preferences: ['hard', 'epic'] as QuestDifficulty[]
    };
    return this.generateQuestsForType(monthlyConfig);
  }
  ...
  static async autoGenerateScheduledQuests(): Promise<void> {
    try {
      ...
      if (now.getUTCHours() === 0 && now.getUTCMinutes() < 5) {
        await this.generateDailyQuestsForAllUsers();
      }
      ...
      if (now.getUTCDate() === 1 && now.getUTCHours() === 0 && now.getUTCMinutes() < 5) {
        await this.generateMonthlyQuestsForAllUsers();
      }

      // Clean up expired quests
      await this.expireOverdueQuests();
    } catch (error) {
  ...
  private static async generateDailyQuestsForAllUsers(): Promise<void> {
    try {
      ...
      for (const user of users || []) {
        ...
        try {
          await this.generateDailyQuests(config);
        } catch (error) {
          console.error(`Error generating daily quests for user ${user.id}:`, error);
        }
      }
    } catch (error) {
  ...
  private static async generateWeeklyQuestsForAllUsers(): Promise<void> {
    try {
      ...
      for (const user of users || []) {
        ...
        try {
          await this.generateWeeklyQuests(config);
        } catch (error) {
          console.error(`Error generating weekly quests for user ${user.id}:`, error);
        }
      }
    } catch (error) {
  ...
  private static async generateMonthlyQuestsForAllUsers(): Promise<void> {
    try {
      ...
      for (const user of users || []) {
        ...
        try {
          await this.generateMonthlyQuests(config);
        } catch (error) {
          console.error(`Error generating monthly quests for user ${user.id}:`, error);
        }
      }
    } catch (error) {
   ...
   */
  private static async generateQuestsForType(config: QuestGenerationConfig): Promise<Quest[]> {
    const { company_id, user_level, character_class, quest_type, difficulty_preferences } = config;
    ...
    if (templateError || !templates?.quest_templates) {
      ...
      return this.generateDefaultQuests(config);
    }
    ...
    const questCount = this.getQuestCount(quest_type);
    const selectedTemplates = this.selectTemplates(templatesToUse, questCount, difficulty_preferences);

    for (const template of selectedTemplates) {
      const quest = await this.createQuestFromTemplate(template, config);
      if (quest) {
  ...
  static async getUserActiveQuests(userId: string): Promise<UserQuest[]> {
    ...
    if (!userValidation.isValid) {
      throw new ValidationError(`Invalid user ID: ${userValidation.errors?.join(', ')}`, 'user_id');
    }
  ...
  static async getUserQuestProgress(userId: string): Promise<QuestProgress[]> {
    ...
    return userQuests.map(userQuest => {
      const current_value = this.getCurrentProgress(userQuest.quest!, userStats);
      const percentage = Math.min((current_value / userQuest.quest!.target_value) * 100, 100);
      ...
      return {
        ...
        status: percentage >= 100 ? 'completed' as QuestStatus : userQuest.status as QuestStatus,
        time_remaining
      };
    });
  }
   ...
   */
  static async updateQuestProgress(userId: string, actionType: 'referral' | 'commission' | 'level' | 'achievement', value: number = 1): Promise<void> {
    ...
    if (!validActionTypes.includes(actionType)) {
      throw new ValidationError(`Invalid action type: ${actionType}`, 'actionType');
    }

    // Validate value to prevent unreasonable progress jumps
    if (typeof value !== 'number' || value < 0 || value > 1000) {
      throw new ValidationError('Progress value must be between 0 and 1000', 'value');
    }
    ...
    for (const userQuest of userQuests) {
      if (this.shouldUpdateQuest(userQuest.quest!, actionType)) {
        const currentProgress = this.getCurrentProgress(userQuest.quest!, userStats);
        const newProgress = Math.min(currentProgress + value, userQuest.quest!.target_value);
        const isCompleted = newProgress >= userQuest.quest!.target_value;
        ...
        if (isCompleted && !userQuest.reward_claimed) {
          await this.distributeQuestRewards(userId, userQuest.quest_id);
        }
      }
    }
  }
   ...
   * Claim quest rewards
   */
  static async claimQuestRewards(userId: string, userQuestId: string): Promise<QuestReward | null> {
  ...
  static async getUserQuestStats(userId: string): Promise<QuestStats> {
    ...
    if (error) {
      ...
      return this.getDefaultQuestStats();
    }

    const completedQuests = userQuests.filter(q => q.is_completed);
    const totalQuests = userQuests.length;

    return {
      total_completed: completedQuests.length,
      completion_rate: totalQuests > 0 ? (completedQuests.length / totalQuests) * 100 : 0,
      average_completion_time: this.calculateAverageCompletionTime(completedQuests),
      favorite_quest_type: this.getFavoriteQuestType(completedQuests),
      total_xp_earned: completedQuests.reduce((sum, q) => {
        // Note: This would need to join with quests table to get reward_xp
        return sum + 100; // Placeholder
      }, 0),
      total_commission_earned: completedQuests.reduce((sum, q) => {
        // Note: This would need to join with quests table to get reward_commission
        return sum + 5; // Placeholder
      }, 0),
      current_streak: this.calculateCurrentStreak(completedQuests),
      longest_streak: this.calculateLongestStreak(completedQuests)
    };
  }

  // Private helper methods
  private static getQuestCount(questType: 'daily' | 'weekly' | 'monthly'): number {
    switch (questType) {
      case 'daily': return 3;
  ...
  private static generateDefaultQuests(config: QuestGenerationConfig): Quest[] {
    ...
    return baseQuests.map((quest, index) => ({
      ...
      end_date: this.getQuestEndDate(config.quest_type),
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    }));
  }

  private static selectTemplates(templates: QuestTemplate[], count: number, preferences?: string[]): QuestTemplate[] {
    let availableTemplates = templates;

    if (preferences && preferences.length > 0) {
      availableTemplates = templates.filter(t => preferences.includes(t.difficulty));
    }
  ...
  private static async createQuestFromTemplate(template: QuestTemplate, config: QuestGenerationConfig): Promise<Quest | null> {
    const targetValue = Math.round(
      template.requirements_template.base_target_value *
      (1 + (config.user_level - 1) * template.requirements_template.scaling_factor)
    );
    ...
    const quest: Quest = {
      ...
      reward_xp: Math.round(template.base_reward_xp * (1 + config.user_level * 0.1)),
      reward_commission: targetValue * 0.1, // 10% of target value
      is_active: true,
      start_date: new Date().toISOString(),
      end_date: this.getQuestEndDate(template.quest_type),
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    };
  ...
  private static getQuestEndDate(questType: 'daily' | 'weekly' | 'monthly' | 'special'): string {
    const now = new Date();
    switch (questType) {
      case 'daily':
  ...
  private static getCurrentProgress(quest: Quest, userStats: any): number {
  ...
  private static async distributeQuestRewards(userId: string, questId: string): Promise<void> {
    // This would handle automatic reward distribution
    // For now, we'll just log it
    console.log(`Distributing rewards for quest ${questId} to user ${userId}`);
  }

  private static calculateAverageCompletionTime(completedQuests: any[]): number {
  ...
  private static getFavoriteQuestType(completedQuests: any[]): string {
    ...
    return Object.entries(typeCount).sort(([,a], [,b]) => b - a)[0]?.[0] || 'none';
  }

  private static calculateCurrentStreak(completedQuests: any[]): number {
    // Simplified streak calculation
    const recentQuests = completedQuests
      .filter(q => new Date(q.completed_at || q.updated_at) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))
      .length;

    return recentQuests > 0 ? recentQuests : 0;
  }

  private static calculateLongestStreak(completedQuests: any[]): number {
    // Simplified longest streak calculation
    return completedQuests.length > 0 ? Math.min(completedQuests.length, 30) : 0;
  }
