...
export const CHARACTER_CLASSES: CharacterClass[] = [
  {
    id: 'scout',
    name: 'Scout',
    description: 'Masters of reach and discovery. Scouts excel at finding new referral opportunities and maximizing visibility.',
    emoji: '🏃',
    color: '#3B82F6', // Blue
    xpMultiplier: 1.2,
  ...
  },
  {
    id: 'sage',
    name: 'Sage',
    description: 'Champions of value and quality. Sages focus on high-value conversions and building lasting relationships.',
    emoji: '🧙',
    color: '#8B5CF6', // Purple
    xpMultiplier: 1.5,
    specialAbility: 'Value Mastery - 50% bonus XP from high-value referrals ($100+)',
    playstyle: 'value',
    requirements: {
      minTotalValue: 500,
    },
  ...
  },
  {
    id: 'champion',
    name: 'Champion',
    description: 'Paragons of volume and consistency. Champions thrive on consistent performance and high referral volumes.',
    emoji: '⚔️',
    color: '#EF4444', // Red
    xpMultiplier: 1.3,
    specialAbility: 'Volume Dominance - 30% bonus XP from referral streaks (3+ in a week)',
    playstyle: 'volume',
    requirements: {
      minReferrals: 5,
    },
...
export class CharacterClassManager {
  ...
  static async recommendClass(userId: string): Promise<ClassRecommendation> {
    try {
      ...
      const analysis = this.analyzeReferralPattern(referrals || []);
  ...
  private static analyzeReferralPattern(referrals: any[]) {
    ...
    const totalValue = referrals.reduce((sum, ref) => sum + (ref.value || 0), 0);
    ...
    const weeksDiff = Math.max(1, (newestReferral.getTime() - oldestReferral.getTime()) / (7 * 24 * 60 * 60 * 1000));
  ...
  private static generateRecommendations(analysis: any): ClassRecommendation[] {
    ...
    if (analysis.referralPattern === 'high_value' || analysis.averageReferralValue > 75) {
    ...
    if (analysis.referralPattern === 'frequent' || analysis.totalReferrals >= 5) {
      recommendations.push({
        classId: 'champion',
    ...
    if (analysis.referralPattern === 'consistent' || analysis.totalReferrals < 5) {
      recommendations.push({
        classId: 'scout',
  ...
  static async assignClass(
    ...
    try {
      const characterClass = this.getClassById(classId);
      if (!characterClass) {
        throw new Error(`Invalid character class: ${classId}`);
      }

      // Check if user meets class requirements
      await this.validateClassRequirements(userId, characterClass);
   ...
   */
  private static async validateClassRequirements(userId: string, characterClass: CharacterClass) {
    ...
    if (characterClass.requirements.minReferrals && completedReferrals.length < characterClass.requirements.minReferrals) {
      throw new Error(`Requires at least ${characterClass.requirements.minReferrals} completed referrals`);
    }

    // Check minimum total value requirement
    if (characterClass.requirements.minTotalValue && totalValue < characterClass.requirements.minTotalValue) {
      throw new Error(`Requires at least $${characterClass.requirements.minTotalValue} in total referral value`);
    }

    // Check maximum referrals requirement (for scout class)
    if (characterClass.requirements.maxReferrals && completedReferrals.length > characterClass.requirements.maxReferrals) {
      throw new Error(`Too many referrals for this class. Maximum: ${characterClass.requirements.maxReferrals}`);
    }
  }
   ...
   */
  static async getUserClass(userId: string): Promise<UserClassAssignment | null> {
    try {
  ...
  static async autoAssignClass(userId: string): Promise<UserClassAssignment> {
    try {
      // Get recommendation
      const recommendation = await this.recommendClass(userId);

      // Assign the recommended class
      const assignment = await this.assignClass(
        userId,
      ...
      console.log(`Auto-assigned class ${recommendation.classId} to user ${userId}`);
      return assignment;
    } catch (error) {
      console.error('Error in auto class assignment:', error);
      // Fallback to scout class
      return await this.assignClass(userId, 'scout', 'auto', 'Fallback assignment');
    }
  }
}