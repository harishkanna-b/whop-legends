...
import { LevelProgression, LevelingManager } from "./leveling";
...
export class UserProfileManager {
	...
	static async getProfile(userId: string): Promise<UserProfile> {
		try {
			...
			// Get user statistics
			const stats = await UserProfileManager.getUserStats(userId);

			// Get achievements
			const achievements = await UserProfileManager.getUserAchievements(userId);

			// Get recent activity
			const recentActivity = await UserProfileManager.getRecentActivity(
				userId,
				10,
			);

			// Get user preferences
			const preferences = await UserProfileManager.getUserPreferences(userId);
	...
	static async getUserStats(userId: string): Promise<UserStats> {
		try {
			...
			const averageReferralValue =
				completedReferrals.length > 0
					? totalValue / completedReferrals.length
					: 0;
			...
			// Calculate streaks
			const { currentStreak, bestStreak } = UserProfileManager.calculateStreaks(
				referrals || [],
			);

			// Get user rank
			const rank = await UserProfileManager.getUserRank(userId);
	 ...
	 */
	private static calculateStreaks(referrals: any[]): {
		currentStreak: number;
		bestStreak: number;
	} {
		...
		const currentStreak = sortedReferrals.filter(
			(ref) => new Date(ref.created_at) > oneWeekAgo,
		).length;
	...
	static async getUserAchievements(userId: string): Promise<Achievement[]> {
		try {
			...
			return (achievementDefs || []).map((def: any) => ({
				id: def.id,
				name: def.name,
				...
				unlockedAt: unlockedSet.has(def.id)
					? userAchievements?.find((ua: any) => ua.achievement_id === def.id)
							?.unlocked_at
					: undefined,
		...
		try {
			...
			(xpEvents || []).forEach((event: any) => {
				activities.push({
					...
					type: UserProfileManager.mapXPEventToActivity(event.type),
			...
			// Add level ups
			(levelHistory || []).forEach((level: any) => {
				activities.push({
			...
			(achievementUnlocks || []).forEach((achievement: any) => {
				activities.push({
			...
			// Add referral events
			(referralHistory || []).forEach((referral: any) => {
				activities.push({
	...
	static async getUserPreferences(userId: string): Promise<UserPreferences> {
		try {
			...
			return UserProfileManager.getDefaultPreferences();
		}
	}
	...
	static async updateProfile(
		...
		try {
			...
			return await UserProfileManager.getProfile(userId);
		} catch (error) {
	...
	static async getPublicProfile(userId: string): Promise<Partial<UserProfile>> {
		try {
			const fullProfile = await UserProfileManager.getProfile(userId);
			const preferences = await UserProfileManager.getUserPreferences(userId);

			// Filter based on privacy settings
			const publicProfile: Partial<UserProfile> = {
				id: fullProfile.id,
				username: preferences.privacy.showProfile
					? fullProfile.username
					: undefined,
				avatarUrl: preferences.privacy.showProfile
				...
				recentActivity: preferences.privacy.showActivity
					? fullProfile.recentActivity
					: [],
			};
