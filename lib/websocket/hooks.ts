...
import { type WebSocketMessage, webSocketService } from "./websocket-service";
...
export const useQuestUpdates = (userId: string) => {
	...
	useEffect(() => {
		if (userId) {
			subscriptionRef.current = webSocketService.subscribeToQuestUpdates(
				userId,
				handleQuestUpdate,
			);
...
export const useAnalyticsUpdates = (companyId: string) => {
			...
			switch (message.type) {
				...
				case "commission_earned":
					return {
						...prev,
						totalCommission:
							prev.totalCommission + (message.payload.amount || 0),
						recentActivity: newActivity,
					};
				case "user_level_up":
					return {
	...
	useEffect(() => {
		if (companyId) {
			subscriptionRef.current = webSocketService.subscribeToAnalyticsUpdates(
				companyId,
...
export const useLeaderboardUpdates = (leaderboardId: string) => {
	...
	useEffect(() => {
		if (leaderboardId) {
			subscriptionRef.current = webSocketService.subscribeToLeaderboardUpdates(
				leaderboardId,
...
export const useUserUpdates = (userId: string) => {
	...
	useEffect(() => {
		if (userId) {
			subscriptionRef.current = webSocketService.subscribeToUserUpdates(
				userId,
				handleUserUpdate,
			);
			setIsConnected(webSocketService.getConnectionStatus());
...
export const useReferralUpdates = (userId: string) => {
	...
	useEffect(() => {
		if (userId) {
			subscriptionRef.current = webSocketService.subscribeToReferralUpdates(
				userId,
...
export const useGuildUpdates = (guildId: string) => {
	...
	useEffect(() => {
		if (guildId) {
			subscriptionRef.current = webSocketService.subscribeToGuildUpdates(
				guildId,
				handleGuildUpdate,
			);
			setIsConnected(webSocketService.getConnectionStatus());
