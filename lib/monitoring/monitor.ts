...
export class MonitoringService {
  ...
  time<T>(name: string, fn: () => Promise<T>, tags: Record<string, string> = {}): Promise<T> {
    ...
    return fn()
      ...
      .catch(error => {
        const duration = Date.now() - startTime;
        this.timer(name, duration, { ...tags, status: 'error', requestId, error: error.message });
  ...
  async runHealthCheck(component: string, checkFn: () => Promise<HealthCheck>): Promise<void> {
    try {
      ...
      logger.error(`Health check failed: ${component}`, error, {component: 'Monitoring'});
    }
  }
  ...
  addAlertRule(rule: AlertRule): void {
    ...
    logger.info(`Alert rule added: ${rule.id}`, {component: 'Monitoring', rule});
  }
  ...
  collectSystemMetrics(): void {
    ...
    this.gauge('system.memory.heapTotal', memUsage.heapTotal, { unit: 'bytes' });
    ...
    // Event loop lag
    this.gauge('system.eventLoop.lag', this.measureEventLoopLag(), { unit: 'ms' });
  ...
  private startHealthChecks(): void {
    ...
    setInterval(() => {
      this.healthChecks.forEach((check, component) => {
        ...
        logger.debug(`Health check due: ${component}`, {component: 'Monitoring'});
      });
    }, 60000);
  }
...
export function monitorPerformance(name: string, tags: Record<string, string> = {}) {
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      return monitoring.time(name, () => originalMethod.apply(this, args), tags);
    };
  ...
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      try {
        ...
        await monitoring.runHealthCheck(component, () => Promise.resolve(result));
        return result;
      } catch (error) {
        await monitoring.runHealthCheck(component, () => Promise.resolve({
          component,
