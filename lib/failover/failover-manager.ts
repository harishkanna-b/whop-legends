...
export class FailoverManager {
	...
	async execute<T>(
		...
		for (
			let attempt = 0;
			attempt <= (options?.skipFailover ? 0 : this.config.maxRetries);
			attempt++
		) {
			try {
	...
	private updateProviderErrorRate(provider: string, success: boolean): void {
		...
		const recentRequests = this.requestHistory.filter(
			(req) => req.provider === provider && Date.now() - req.timestamp < 300000, // 5 minutes
		);
	...
	private async checkAllProviders(): Promise<void> {
		const healthCheckPromises = Array.from(this.providers.keys()).map(
			(provider) => this.performHealthCheck(provider),
		);
	...
	private evaluateFailoverNeeds(healthResults: HealthCheckResult[]): void {
		...
		const currentHealth = healthResults.find(
			(r) => r.provider === this.currentProvider,
		);
		...
		if (currentHealth && !currentHealth.isHealthy) {
			...
			currentProviderData.consecutiveSuccesses >= this.config.failbackThreshold
		) {
	...
	private initiateFailover(): void {
		...
		if (availableProviders.length > 0) {
			...
			if (newProvider !== this.currentProvider) {
				console.log(
					`Failover initiated: ${this.currentProvider} -> ${newProvider}`,
				);
	...
	private initiateFailback(): void {
		...
		if (this.currentProvider !== this.config.primaryProvider) {
			console.log(
				`Failback initiated: ${this.currentProvider} -> ${this.config.primaryProvider}`,
			);
			this.currentProvider = this.config.primaryProvider;
	...
	forceFailover(provider?: string): void {
		const targetProvider =
			provider || this.getNextAvailableProvider(this.currentProvider);
