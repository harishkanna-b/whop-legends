...
export default function WhopDevWrapper({
	children,
	fallback,
}: WhopDevWrapperProps) {
	...
	useEffect(() => {
		...
		const handleWhopError = (event: ErrorEvent) => {
			if (
				event.message?.includes("App API Key") ||
				event.message?.includes("user token")
			) {
				setError(
				...
				<div className="max-w-md w-full bg-white shadow-lg rounded-lg p-6">
					<div className="text-center">
						...
						<div className="space-y-2 text-sm text-gray-600">
							<p>
								📋 <strong>Development Mode:</strong> This app is designed to
								run within the Whop.com ecosystem.
							</p>
							<p>
								🔧 <strong>To test locally:</strong> You can view the static
								components, but full functionality requires Whop authentication.
							</p>
							<p>
								🌐 <strong>For full experience:</strong> Access this app through
								your Whop dashboard.
							</p>
						</div>
						{fallback && (
						...
						<div className="flex items-center justify-between">
							<div>
								<h1 className="text-xl font-bold">
									🎮 Whop Legends - Development Mode
								</h1>
								<p className="text-blue-100 text-sm">
									Running outside Whop iframe - Limited functionality
								</p>
							</div>
