...
export const DEFAULT_ACHIEVEMENTS: Achievement[] = [
  // Referral Achievements
  {
    ...
    points: 50,
    requirements: [
      { type: 'referrals', target: 1, operator: 'greater_equal', description: 'Make 1 referral' }
    ],
    rewards: [
  ...
  },
  {
    ...
    points: 200,
    requirements: [
      { type: 'referrals', target: 5, operator: 'greater_equal', description: 'Make 5 referrals' }
    ],
    rewards: [
  ...
  },
  {
    ...
    points: 1000,
    requirements: [
      { type: 'referrals', target: 25, operator: 'greater_equal', description: 'Make 25 referrals' }
    ],
    rewards: [
      { type: 'xp', value: 2500, description: '2500 XP bonus' },
      { type: 'title', value: 'Referral Master', description: 'Prestigious title' },
      { type: 'badge', value: 'master_referrer', description: 'Exclusive badge' }
    ],
    isActive: true,
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString()
  ...
  // Level Achievements
  {
    ...
    points: 100,
    requirements: [
      { type: 'level', target: 5, operator: 'greater_equal', description: 'Reach level 5' }
    ],
    rewards: [
  ...
  },
  {
    ...
    points: 250,
    requirements: [
      { type: 'level', target: 10, operator: 'greater_equal', description: 'Reach level 10' }
    ],
    rewards: [
      { type: 'xp', value: 500, description: '500 XP bonus' },
  ...
  },
  {
    ...
    points: 500,
    requirements: [
      { type: 'level', target: 25, operator: 'greater_equal', description: 'Reach level 25' }
    ],
    rewards: [
      { type: 'xp', value: 1000, description: '1000 XP bonus' },
      { type: 'title', value: 'Legend in the Making', description: 'Prestigious title' },
  ...
  // Value Achievements
  {
    ...
    points: 300,
    requirements: [
      { type: 'value', target: 1000, operator: 'greater_equal', description: 'Generate $1000 in value' }
    ],
    rewards: [
      { type: 'xp', value: 750, description: '750 XP bonus' }
    ],
    isActive: true,
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString()
  },
  {
    ...
    points: 1000,
    requirements: [
      { type: 'value', target: 10000, operator: 'greater_equal', description: 'Generate $10,000 in value' }
    ],
    rewards: [
      { type: 'xp', value: 2500, description: '2500 XP bonus' },
      { type: 'title', value: 'Value Master', description: 'Prestigious title' },
  ...
  // Streak Achievements
  {
    ...
    points: 300,
    requirements: [
      { type: 'streak', target: 7, operator: 'greater_equal', description: '7-day streak' }
    ],
    rewards: [
      { type: 'xp', value: 500, description: '500 XP bonus' },
  ...
  },
  {
    ...
    points: 1000,
    requirements: [
      { type: 'streak', target: 30, operator: 'greater_equal', description: '30-day streak' }
    ],
    rewards: [
      { type: 'xp', value: 2000, description: '2000 XP bonus' },
      { type: 'title', value: 'Monthly Master', description: 'Prestigious title' },
...
export class AchievementManager {
  ...
  async getAchievementProgress(userId: string): Promise<AchievementProgress[]> {
    try {
      ...
      return achievements.map(achievement => {
        ...
        const maxProgress = userAchievement?.maxProgress || achievement.maxProgress || 1
  ...
  async updateProgress(
    userId: string,
    eventType: 'referral' | 'level_up' | 'xp_gained' | 'value_generated' | 'streak_updated',
    value: number,
    metadata?: any
  ): Promise<void> {
    try {
      ...
      for (const achievement of relevantAchievements) {
        ...
        if (!userAchievement) {
          ...
          await this.createUserAchievement(userId, achievement.id, value, achievement.maxProgress || mainRequirement.target)
        } else if (!userAchievement.unlockedAt) {
          // Update existing achievement progress
          const newProgress = Math.min(value, achievement.maxProgress || mainRequirement.target)
          ...
          if (this.checkRequirement(newProgress, mainRequirement)) {
            await this.unlockAchievement(userId, userAchievement.id, achievement)
          }
        }
      }
    } catch (error) {
  ...
  private async createUserAchievement(userId: string, achievementId: string, progress: number, maxProgress: number): Promise<void> {
    try {
      const { data, error } = await supabase
        .from('user_achievements')
        .insert({
      ...
      if (achievement && this.checkRequirement(progress, achievement.requirements[0])) {
  ...
  private async unlockAchievement(userId: string, userAchievementId: string, achievement: Achievement): Promise<void> {
    try {
      await supabase
        .from('user_achievements')
        .update({
          unlocked_at: new Date().toISOString(),
          progress: achievement.maxProgress || achievement.requirements[0].target
        })
  ...
  private async grantRewards(userId: string, achievement: Achievement): Promise<void> {
    try {
  ...
  private checkRequirement(value: number, requirement: AchievementRequirement): boolean {
  ...
  async getAchievementStats(userId: string): Promise<{
    ...
    try {
      ...
      for (const category of ACHIEVEMENT_CATEGORIES) {
        const categoryAchievements = achievements.filter(a => a.category.id === category.id)
      ...
      for (const rarity of ACHIEVEMENT_RARITIES) {
        ...
        const rarityUnlocked = rarityAchievements.filter(a => unlockedIds.has(a.id))
  ...
  async getRecentAchievements(userId: string, limit: number = 10): Promise<Array<{
